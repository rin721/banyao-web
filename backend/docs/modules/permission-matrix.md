...
Community 模块当前提供公开读取、轻量公开评论 / 弹幕发布、匿名创作者关注、匿名视频互动、视频级举报提交和匿名通知收件箱，供 Nuxt 视频社区在关闭 mock 后接入。真实投稿、评论 / 弹幕审核、举报处理后台、登录态关系归并、外部通知投递、登录态消息中心和创作者后台管理还不是后端生产能力，前端相关能力只能作为后端不可用时的浏览器降级或后续任务说明。
