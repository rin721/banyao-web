...
| 社区匿名关注 | `POST/DELETE /api/v1/public/community/users/:handle/follow` | 公开 | 无权限码 | `frontend/` 创作者卡片、用户页、关注动态 | 以匿名 `clientId` 写入后端关注关系；后续登录态归并、通知和黑名单需另行设计权限与状态 |
| 社区匿名视频互动 | `GET /api/v1/public/community/videos/:idOrSlug/interaction-state`、`POST/DELETE /api/v1/public/community/videos/:idOrSlug/interactions/:kind`、`GET /api/v1/public/community/library` | 公开 | 无权限码 | `frontend/` 视频页点赞 / 收藏 / 稍后看、稍后看和收藏列表 | 以匿名 `clientId` 写入后端互动关系；后续登录态归并和通知需另行设计权限与状态 |

Community 模块当前提供公开读取、轻量公开评论 / 弹幕发布、匿名创作者关注和匿名视频互动，供 Nuxt 视频社区在关闭 mock 后接入。真实投稿、评论 / 弹幕审核、举报、登录态关系归并、通知和创作者后台管理还不是后端生产能力，前端相关能力只能作为后端不可用时的浏览器降级或后续任务说明。
