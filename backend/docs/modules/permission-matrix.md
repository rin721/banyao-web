...
| 社区账号会话 | `/api/v1/public/community/auth/*` | 公开 / 社区会话 | 登出要求社区会话和 CSRF | `frontend/` 登录、注册、账号状态 | 响应只暴露社区账号字段：`userId`、`sessionId`、过期时间和 `account.id / account.handle / account.displayName` |
...
| 社区动态 | `GET /api/v1/public/community/dynamics`、`POST /api/v1/public/community/dynamics`、`POST /api/v1/public/community/account/dynamics` | 公开 / 社区会话 | 账号路径要求社区会话和 CSRF | `frontend/` 首页动态流、关注动态发布框 | 匿名动态使用 `clientId`；社区账号动态使用 `account.displayName` 展示作者名 |
...
| 社区投稿审核 | `GET /api/v1/community/submissions`、`PATCH /api/v1/community/submissions/:submissionId/review` | `tenant` | `community_submission:review` | 当前为后端受控 API，后台可视化页面待后续节点 | 主系统 IAM 权限保护；支持审核队列、`approved` / `rejected` / `published` 状态、审核备注、审核时间和发布视频 ID 回写，不创建媒体文件 |
...
Community 模块提供公开读取、评论 / 弹幕 / 动态发布、创作者关注、视频互动、观看历史、投稿元数据、视频级举报和通知收件箱。匿名流程使用浏览器 `clientId`，社区账号流程使用当前会话派生的账号范围 `clientId`；前端在关闭 mock 后通过 `/api/v1/public/community/*` 接入这些接口。
