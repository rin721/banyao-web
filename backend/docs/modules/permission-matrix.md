...
Community 模块提供公开读取、评论 / 弹幕 / 动态发布、创作者关注、视频互动、观看历史、投稿元数据、视频级举报、通知收件箱，以及后台社区账号、投稿审核和举报处理。匿名流程使用浏览器 `clientId`，社区账号流程使用当前社区会话派生的账号范围 `clientId`；前端在关闭 mock 后通过 `/api/v1/public/community/*` 接入社区前台接口，后台 React WebUI 通过 `/api/v1/community/*` 接入受 IAM 保护的真实管理能力。创作者中心、活动配置和批量评论治理尚无后端契约，不在当前控制台伪造页面。
