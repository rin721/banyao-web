...
| 社区公开弹幕发布 | `POST /api/v1/public/community/videos/:idOrSlug/danmaku` | 公开 | 无权限码 | `frontend/` 视频播放器弹幕输入框 | 当前轻量发布不绑定登录态；后续审核、举报、屏蔽和用户关系归并需另行设计权限与状态 |
| 社区公开动态 | `GET /api/v1/public/community/dynamics`、`POST /api/v1/public/community/dynamics` | 公开 | 无权限码 | `frontend/` 首页动态流、关注动态发布框 | 以匿名 `clientId` 读取和发布轻量动态，可绑定视频；后续登录态创作者身份、审核、编辑删除和风控需另行设计 |
| 社区匿名关注 | `POST/DELETE /api/v1/public/community/users/:handle/follow` | 公开 | 无权限码 | `frontend/` 创作者卡片、用户页、关注动态 | 以匿名 `clientId` 写入后端关注关系，关注成功会写入匿名通知；后续登录态归并和黑名单需另行设计权限与状态 |
| 社区匿名视频互动 | `GET /api/v1/public/community/videos/:idOrSlug/interaction-state`、`POST/DELETE /api/v1/public/community/videos/:idOrSlug/interactions/:kind`、`GET /api/v1/public/community/library` | 公开 | 无权限码 | `frontend/` 视频页点赞 / 收藏 / 稍后看、稍后看和收藏列表 | 以匿名 `clientId` 写入后端互动关系，新互动会写入匿名通知；后续登录态归并需另行设计权限与状态 |
| 社区公开视频举报 | `POST /api/v1/public/community/videos/:idOrSlug/reports` | 公开 | 无权限码 | `frontend/` 视频页稿件举报弹窗 | 以匿名 `clientId` 写入后端待处理举报记录，并写入匿名通知回执；审核处理、误报关闭和后台权限需另行设计 |
| 社区匿名通知 | `GET /api/v1/public/community/notifications`、`POST /api/v1/public/community/notifications/read` | 公开 | 无权限码 | `frontend/` 通知页面、移动端顶部通知入口 | 以匿名 `clientId` 查询评论、弹幕、关注、视频互动和举报回执消息，标记已读只影响当前匿名客户端 |

Community 模块当前提供公开读取、轻量公开评论 / 弹幕 / 动态发布、匿名创作者关注、匿名视频互动、视频级举报提交和匿名通知收件箱，供 Nuxt 视频社区在关闭 mock 后接入。真实投稿、评论 / 弹幕 / 动态审核、举报处理后台、登录态关系归并、外部通知投递、登录态消息中心和创作者后台管理还不是后端生产能力，前端相关能力只能作为后端不可用时的浏览器降级或后续任务说明。
