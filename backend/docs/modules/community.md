...
`internal/modules/community` 为 `frontend/` Nuxt 视频社区提供公开只读数据接口。当前模块覆盖首页聚合、分类树、视频列表、视频详情、弹幕、搜索、创作者资料和关注推荐预览。
...
- 本模块是公开只读模块，不提供 IAM 权限码，也不写入 `system_apis` 的受保护权限目录。
- 当前不处理投稿审核、真实关注关系、评论写入、点赞收藏写入或创作者后台管理。
- 前端的本地关注、评论、历史、收藏和投稿草稿仍是浏览器本地体验层，不能描述为后端已生产持久化。
