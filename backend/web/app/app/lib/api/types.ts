...
export type RegistrationMode = "disabled" | "direct" | "email_verification" | "invite_only";
...
export type AnnouncementStatus = BackendString<"archived" | "draft" | "published">;
...
export type PublicAnnouncementPage = {
...
export type IAMNotificationOutboxKind = BackendString<
  "email_verification" | "invitation" | "password_reset"
>;

export type IAMNotificationOutboxStatus = BackendString<"failed" | "pending" | "sent">;

export type IAMNotificationOutboxItem = {
  attempts: number;
  createdAt: string;
  id: number | string;
  kind: IAMNotificationOutboxKind;
  lastError: string;
  maxAttempts: number;
  ...
  resourceType: string;
  sentAt?: string | null;
  status: IAMNotificationOutboxStatus;
  updatedAt: string;
};

export type IAMNotificationOutboxPage = {
  items: IAMNotificationOutboxItem[];
  page: number;
  pageSize: number;
  storageStatus: string;
  total: number;
