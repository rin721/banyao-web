...
const zhCN = JSON.parse(
  ...
  admin: {
    ...
    roles: {
      ...
      states: {
        writePermissionDescription: string;
      };
      title: string;
    };
    organizations: {
    ...
    operationRecords: {
      ...
      states: {
        deletePermissionDescription: string;
      };
      title: string;
    };
    errorLogs: {
...
type NotificationOutboxTestMessages = {
  actions: {
    retry: string;
    search: string;
  };
...
const zhNotificationOutbox = (
  zhCN as typeof zhCN & {
    admin: typeof zhCN.admin & {
      notificationOutbox: NotificationOutboxTestMessages;
    };
  }
).admin.notificationOutbox;
...
async function preferZhCN(page: Page) {
  await page.addInitScript(() => {
    window.localStorage.setItem("console-locale", "zh-CN");
  });
}
...
async function setAuthenticatedSession(
  ...
  await page.route(/\/api\/v1\/me\/session$/, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        code: 0,
        data: sessionSnapshot(accessToken, sessionOverrides),
      }),
    });
  });
...
function sessionSnapshot(
  ...
  const orgId = claimString(overrides.orgId ?? claims.orgId, "1");
  const productCode =
    typeof overrides.productCode === "string" ? overrides.productCode : "console-platform";
  return {
    accessExpiresAt: "2099-01-01T00:00:00Z",
    clientType: "pc_web",
    orgId,
    permissions: [
      { code: "org:read", productCode, scope: "platform" },
      { code: "org:create", productCode, scope: "platform" },
      { code: "org:update", productCode, scope: "tenant" },
      { code: "permission:read", productCode, scope: "platform" },
      { code: "permission:sync", productCode, scope: "platform" },
      { code: "config:read", productCode, scope: "platform" },
      { code: "config:update", productCode, scope: "platform" },
      { code: "traffic_hijack:read", productCode, scope: "platform" },
      { code: "traffic_hijack:update", productCode, scope: "platform" },
      { code: "traffic_hijack:delete", productCode, scope: "platform" },
      { code: "role:read", productCode, scope: "tenant" },
      { code: "role:create", productCode, scope: "tenant" },
      { code: "role:update", productCode, scope: "tenant" },
      { code: "user:read", productCode, scope: "tenant" },
      { code: "user:invite", productCode, scope: "tenant" },
      { code: "user:update", productCode, scope: "tenant" },
      { code: "parameter:read", productCode, scope: "platform" },
      { code: "parameter:create", productCode, scope: "platform" },
      { code: "parameter:update", productCode, scope: "platform" },
      { code: "parameter:delete", productCode, scope: "platform" },
      { code: "operation:read", productCode, scope: "platform" },
      { code: "operation:delete", productCode, scope: "platform" },
      { code: "version:read", productCode, scope: "platform" },
      { code: "version:create", productCode, scope: "platform" },
      { code: "version:import", productCode, scope: "platform" },
      { code: "version:delete", productCode, scope: "platform" },
      { code: "version:download", productCode, scope: "platform" },
      { code: "dictionary:read", productCode, scope: "platform" },
      { code: "dictionary:create", productCode, scope: "platform" },
      { code: "dictionary:update", productCode, scope: "platform" },
      { code: "dictionary:delete", productCode, scope: "platform" },
      { code: "media:read", productCode, scope: "platform" },
      { code: "media:update", productCode, scope: "platform" },
      { code: "media:upload", productCode, scope: "platform" },
      { code: "media:import", productCode, scope: "platform" },
      { code: "media:delete", productCode, scope: "platform" },
      { code: "media:download", productCode, scope: "platform" },
      { code: "api_token:read", productCode, scope: "tenant" },
      { code: "api_token:create", productCode, scope: "tenant" },
      ...
      { code: "announcement:create", productCode, scope: "tenant" },
      { code: "announcement:update", productCode, scope: "tenant" },
      { code: "announcement:delete", productCode, scope: "tenant" },
    ],
    productCode,
    refreshExpiresAt: "2099-01-02T00:00:00Z",
    sessionId: claimString(overrides.sessionId ?? claims.sessionId, `sess-${orgId}`),
    userId: claimString(overrides.userId ?? claims.userId, "1"),
    ...overrides,
  };
}

function claimString(value: unknown, fallback: string) {
...
function authFromHeaders(headers: Record<string, string>) {
  ...
  const accessToken = cookieValue(headers.cookie ?? "", "console_access");
...
function responseCookies(accessToken: string, refreshToken = "refresh-token") {
  return {
    "Set-Cookie": [
      `console_access=${accessToken}; Path=/; HttpOnly; SameSite=Lax; Expires=Thu, 01 Jan 2099 00:00:00 GMT`,
      `console_refresh=${refreshToken}; Path=/; HttpOnly; SameSite=Lax; Expires=Fri, 02 Jan 2099 00:00:00 GMT`,
      `console_csrf=csrf-token; Path=/; SameSite=Lax; Expires=Thu, 01 Jan 2099 00:00:00 GMT`,
    ].join("\n"),
  };
}
...
async function scrollTableActionsIntoView(row: Locator) {
  await row.locator(".console-user-actions").evaluate((element) => {
    element.scrollIntoView({ block: "center", inline: "end" });
    const scroller = element.closest(".console-data-table-wrap");
...
test("public home renders", async ({ page }) => {
  ...
  await expect(page.locator(".console-brand")).toContainText("Demo Console");
  await expect(page).toHaveTitle(new RegExp(escapeRegExp(en.seo.home.title)));
  await expect(page.locator('meta[name="description"]')).toHaveAttribute(
    "content",
    en.seo.home.description,
  );
  await expect(page.locator('meta[property="og:title"]')).toHaveAttribute("content", /Console/);
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
    "href",
    /http:\/\/127\.0\.0\.1:3002\/$/,
  );
  const homeJsonLd = await page
    .locator('script[data-console-jsonld="home"]')
...
test("public announcements route reads published product-line content", async ({ page }) => {
  ...
  const publishedAnnouncements: PublicAnnouncement[] = [
    {
      content: "公开产品线内容\n\n这条内容由主平台后台发布，再通过公开契约读取。",
      id: "701",
  ...
  await page.route("**/api/v1/public/announcements**", async (route) => {
    ...
    if (request.method() === "GET" && url.pathname === "/api/v1/public/announcements") {
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({
          code: 0,
    ...
    if (request.method() === "GET" && url.pathname === "/api/v1/public/announcements/701") {
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({ code: 0, data: publishedAnnouncements[0] }),
      });
      return;
    }
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: zhCN.site.announcements.title }),
  ).toBeVisible();
  await expect(page.locator(".console-nav").getByText(zhCN.site.nav.announcements)).toBeVisible();
  await page.getByLabel(zhCN.site.announcements.search.label).fill("产品线");
  await page.getByRole("button", { name: zhCN.site.announcements.search.submit }).press("Enter");
  await expect(page.getByRole("heading", { level: 2, name: "公开产品线公告" })).toBeVisible();
  await page.getByRole("link", { name: zhCN.site.announcements.actions.read }).click();
  await expect(page.getByRole("heading", { level: 1, name: "公开产品线公告" })).toBeVisible();
  await expect(page.getByText(zhCN.site.announcements.detail.badge)).toBeVisible();
  await expect(page.getByText("这条内容由主平台后台发布，再通过公开契约读取。")).toBeVisible();
  await expect(
    page.getByRole("link", { name: zhCN.site.announcements.actions.backToList }),
  ).toBeVisible();
  ...
  expect(publicRequests).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        ...
        query: expect.objectContaining({ keyword: "产品线", page: "1", pageSize: "6" }),
      }),
      expect.objectContaining({
        authorization: null,
        locale: "zh-CN",
        method: "GET",
        path: "/api/v1/public/announcements/701",
      }),
    ]),
  );
});
...
test("blog detail uses article metadata from front matter", async ({ page }) => {
  ...
  await expect(page.locator(".console-article-cover")).toBeVisible();
  ...
  const articleJsonLd = await page
    .locator('script[data-console-jsonld="blog-article"]')
    .evaluate((script) => script.textContent ?? "");
...
test("admin route requires an authenticated session", async ({ page }) => {
  await page.addInitScript(() => {
    window.sessionStorage.removeItem("console-platform-session");
  });
  await page.goto("/admin");
  await expect(page).toHaveURL(/\/login\?next=%2Fadmin/);
  await expect(page.locator(".console-platform-sidebar")).toHaveCount(0);
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
});
...
test("authenticated admin dashboard reads backend-supported overview APIs", async ({ page }) => {
  ...
  const trafficTarget: SystemTrafficProbeTarget = {
    ...
    emailRecipients: "",
    enabled: true,
    expectedContentKeyword: "Console",
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          build: {
            goVersion: "go1.25.7",
            module: "github.com/open-console/console-platform",
            path: "github.com/open-console/console-platform/cmd/console",
            settings: [],
            version: "dev",
          },
...
test("admin IAM route renders backend IAM overview read-only", async ({ page }) => {
  ...
  await expect(
    page
      .locator(".console-iam-table--roles")
...
test("admin organizations route manages backend-supported organizations", async ({ page }) => {
  ...
  const createPanel = page.locator(".console-admin-panel").filter({
  ...
  const currentPanel = page.locator(".console-admin-panel").filter({
  ...
  await expect
    .poll(
      async () =>
        (await page.context().cookies()).find((cookie) => cookie.name === "console_access")
          ?.value ?? "",
    )
    .toBe(switchedAccessToken);

  const filterPanel = page.locator(".console-admin-panel").filter({
...
test("admin organizations route disables writes without organization grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "org:read", productCode: "console-platform", scope: "platform" }],
  });
  let writeRequests = 0;
  const organizationRecords: IAMOrganization[] = [
    {
      code: "root",
      createdAt: "2026-06-18T07:00:00Z",
      id: "1",
  ...
  await page.route("**/api/v1/orgs**", async (route) => {
    ...
    if (request.method() !== "GET") {
      ...
      await route.fulfill({
        contentType: "application/json",
        status: 403,
        body: JSON.stringify({ code: 403, message: "permission denied" }),
      });
      return;
    }
    await route.fulfill({
      ...
      body: JSON.stringify({
        code: 0,
        data: {
          items: organizationRecords,
          page: 1,
          pageSize: 10,
          storageStatus: "persisted",
          total: organizationRecords.length,
        },
      }),
    });
  });
  ...
  const permissionTitle = (scope: string, permission: string) =>
    zhCN.admin.organizations.states.writePermissionDescription
      .replace("{{scope}}", scope)
      .replace("{{permission}}", permission);
  const currentPanel = page.locator(".console-admin-panel").filter({
    has: page.getByRole("heading", { name: zhCN.admin.organizations.current.title }),
  });
  await expect(currentPanel.getByLabel(zhCN.admin.organizations.fields.name)).toBeDisabled();
  const saveButton = currentPanel.getByRole("button", {
    name: zhCN.admin.organizations.actions.save,
  });
  await expect(saveButton).toBeDisabled();
  await expect(saveButton).toHaveAttribute("title", permissionTitle("tenant", "org:update"));

  const createPanel = page.locator(".console-admin-panel").filter({
    has: page.getByRole("heading", { name: zhCN.admin.organizations.create.title }),
  });
  await expect(createPanel.getByLabel(zhCN.admin.organizations.fields.code)).toBeDisabled();
  await expect(createPanel.getByLabel(zhCN.admin.organizations.fields.name)).toBeDisabled();
  const createButton = createPanel.getByRole("button", {
    name: zhCN.admin.organizations.actions.create,
  });
  await expect(createButton).toBeDisabled();
  await expect(createButton).toHaveAttribute("title", permissionTitle("platform", "org:create"));
...
test("admin users route manages backend-supported users and invitations", async ({ page }) => {
  ...
  const userTable = page.locator(".console-user-table");
  ...
  const inviteForm = page.locator(".console-user-invite-form");
  await expect(
    inviteForm.getByLabel(zhCN.admin.users.invite.role).locator('option[value="platform_owner"]'),
  ).toHaveCount(0);
  ...
  const deliveryPanel = page.locator(".console-user-delivery-panel");
  await expect(
  ...
  await userTable.locator(".console-data-table-wrap").evaluate((element) => {
    element.scrollLeft = element.scrollWidth;
  });
  const ownerRow = userTable.getByRole("row").filter({ hasText: "Owner User" });
  const ownerRoleSelect = ownerRow.getByLabel(
    zhCN.admin.users.actions.roleSelect.replace("{{name}}", "Owner User"),
    { exact: true },
  );
  await expect(ownerRoleSelect).toBeDisabled();
  await expect(ownerRoleSelect).toHaveValue("");
  await expect(
    ownerRow.locator("button").filter({ hasText: zhCN.admin.users.actions.saveRole }),
  ).toBeDisabled();
  await expect(
    ownerRow.locator("button").filter({ hasText: zhCN.admin.users.actions.disable }),
  ).toBeDisabled();
  ...
  await expect(
    memberRow
      ...
      .locator('option[value="platform_owner"]'),
  ).toHaveCount(0);
  await memberRow
    .getByLabel(zhCN.admin.users.actions.roleSelect.replace("{{name}}", "Member User"), {
      exact: true,
  ...
  await page
    .locator(".console-user-invitation-table .console-data-table-wrap")
    .evaluate((element) => {
      element.scrollLeft = element.scrollWidth;
    });
  await page
  ...
  const filterForm = page.locator(".console-user-filter-form");
...
test("admin users route disables writes without user mutation grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "user:read", productCode: "console-platform", scope: "tenant" }],
  });
  let writeRequests = 0;
  const userRecords: IAMOrganizationUser[] = [
    {
      membershipStatus: "active",
      roles: ["role:member"],
      user: {
  ...
  const invitationRecords: IAMInvitation[] = [
    {
      createdAt: "2026-06-18T08:00:00Z",
      email: "pending@example.com",
      expiresAt: "2099-01-01T00:00:00Z",
  ...
  await expect(page.getByRole("heading", { level: 1, name: zhCN.admin.users.title })).toBeVisible();
  const permissionTitle = (permission: string) =>
    zhCN.admin.users.states.writePermissionDescription.replace("{{permission}}", permission);
  const inviteButton = page.getByRole("button", { name: zhCN.admin.users.actions.invite });
  await expect(inviteButton).toBeDisabled();
  await expect(inviteButton).toHaveAttribute("title", permissionTitle("user:invite"));
  ...
  await scrollTableActionsIntoView(memberRow);
  await expect(
    memberRow.getByLabel(zhCN.admin.users.actions.roleSelect.replace("{{name}}", "Member User"), {
      exact: true,
    }),
  ).toBeDisabled();
  const saveRoleButton = memberRow.getByRole("button", {
    name: zhCN.admin.users.actions.saveRoleFor.replace("{{name}}", "Member User"),
  });
  await expect(saveRoleButton).toBeDisabled();
  await expect(saveRoleButton).toHaveAttribute("title", permissionTitle("user:update"));
  const statusButton = memberRow.getByRole("button", {
    name: zhCN.admin.users.actions.toggleStatusFor
      .replace("{{name}}", "Member User")
      .replace("{{status}}", zhCN.admin.users.status.disabled),
  });
  await expect(statusButton).toBeDisabled();
  await expect(statusButton).toHaveAttribute("title", permissionTitle("user:update"));
  ...
  const revokeInvitationButton = page.getByRole("button", {
    name: zhCN.admin.users.invitation.revokeInvitation.replace("{{email}}", "pending@example.com"),
  });
  await expect(revokeInvitationButton).toBeDisabled();
  await expect(revokeInvitationButton).toHaveAttribute("title", permissionTitle("user:invite"));
  expect(writeRequests).toBe(0);
});
...
test("admin roles route creates and updates backend-supported roles", async ({ page }) => {
  ...
  const roleTable = page.locator(".console-role-table");
  ...
  const createPermissionPanel = createPanel.locator(".console-role-permission-panel").filter({
    has: page.getByRole("heading", { name: zhCN.admin.roles.permissions.createTitle }),
  });
  await expect(
    createPermissionPanel.getByLabel(
      zhCN.admin.roles.permissions.toggle.replace("{{code}}", "config:read"),
    ),
  ).toHaveCount(0);
  ...
  const editPanel = page.locator(".console-admin-panel").filter({
  ...
  const editPermissionPanel = editPanel.locator(".console-role-permission-panel").filter({
...
test("admin roles route disables writes without role mutation grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [
      { code: "role:read", productCode: "console-platform", scope: "tenant" },
      { code: "permission:read", productCode: "console-platform", scope: "tenant" },
    ],
  });
  let writeRequests = 0;
  const roleRecords: IAMRole[] = [
  ...
  const permissionTitle = (permission: string) =>
    zhCN.admin.roles.states.writePermissionDescription.replace("{{permission}}", permission);
  const createButton = page.getByRole("button", { name: zhCN.admin.roles.actions.create });
  await expect(createButton).toBeDisabled();
  await expect(createButton).toHaveAttribute("title", permissionTitle("role:create"));

  const editPanel = page.locator(".console-admin-panel").filter({
    has: page.getByRole("heading", { level: 2, name: zhCN.admin.roles.edit.title }),
  });
  await expect(editPanel.getByLabel(zhCN.admin.roles.fields.name)).toBeDisabled();
  await expect(editPanel.getByLabel(zhCN.admin.roles.fields.description)).toBeDisabled();
  await expect(
    editPanel.getByLabel(zhCN.admin.roles.permissions.toggle.replace("{{code}}", "user:update")),
  ).toBeDisabled();
  const saveButton = editPanel.getByRole("button", { name: zhCN.admin.roles.actions.save });
  await expect(saveButton).toBeDisabled();
  await expect(saveButton).toHaveAttribute("title", permissionTitle("role:update"));
  expect(writeRequests).toBe(0);
});
...
test("admin sessions route renders backend session records with filters and revocation", async ({
  ...
  const sessionTable = page.locator(".console-session-table");
...
test("admin sessions route disables revocation without session revoke grant", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1", { sessionId: "sess-current" });
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "session:read", productCode: "console-platform", scope: "tenant" }],
  });
  let writeRequests = 0;
  ...
  const permissionTitle = zhCN.admin.sessions.states.writePermissionDescription.replace(
    "{{permission}}",
    "session:revoke",
  );
  const revokeButton = sessionTable
    .getByRole("row")
    .filter({ hasText: "sess-readonly" })
    .getByRole("button", {
      name: zhCN.admin.sessions.actions.revokeSession.replace("{{id}}", "sess-readonly"),
    });
  await expect(revokeButton).toBeDisabled();
  await expect(revokeButton).toHaveAttribute("title", permissionTitle);
  await expect(
    sessionTable.getByRole("button", {
      name: zhCN.admin.sessions.actions.revokeSession.replace("{{id}}", "sess-current"),
    }),
...
test("admin security route manages backend-supported MFA setup and logout", async ({ page }) => {
  ...
  const otpauthUrl =
    "otpauth://totp/Console%20Platform:owner@example.com?secret=JBSWY3DPEHPK3PXP&issuer=Console%20Platform";
  ...
  await expect(
    page.locator(".console-security-grid").getByText("owner@example.com", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText("sess-current", { exact: true })).toBeVisible();
  await expect(
    page.locator(".console-iam-status").getByText(zhCN.admin.security.mfa.disabledBadge, {
      exact: true,
    }),
  ).toBeVisible();
  ...
  await expect(
    page.locator(".console-iam-status").getByText(zhCN.admin.security.mfa.enabledBadge, {
      exact: true,
    }),
  ).toBeVisible();
  ...
  await expect
    .poll(() => page.evaluate(() => window.sessionStorage.getItem("console-platform-session")))
    .toBeNull();
...
test("admin design system route edits local theme drafts without backend theme calls", async ({
  ...
  await expect(
    page.locator(".console-theme-package-card").getByText("builtin/default", { exact: true }),
  ).toBeVisible();
  ...
  await expect(page.locator(".console-theme-preview")).toHaveAttribute("data-mode", "dark");
  ...
  await expect(page.locator(".console-theme-color-field code").first()).toHaveText("#2f6f6a");
  await expect(
...
test("admin API tokens route issues and revokes backend API tokens", async ({ page }) => {
  ...
  const tokenTable = page.locator(".console-api-token-table");
  await expect(tokenTable.getByText("Owner User", { exact: true }).first()).toBeVisible();
  await expect(tokenTable.getByText("cp_pat_live", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText("cp_pat_secret_new", { exact: true })).toBeVisible();
  ...
  expect(storageDump).not.toContain("cp_pat_secret_new");
...
test("admin API tokens route disables writes without API token grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "api_token:read", productCode: "console-platform", scope: "tenant" }],
  });
  let writeRequests = 0;
  const tokenRecords: IAMAPIToken[] = [
    {
      createdAt: "2026-06-18T08:00:00Z",
      createdBy: "1",
      expiresAt: "2026-12-31T00:00:00Z",
      ...
      lastUsedIpAddress: "",
      orgId: "1",
      remark: "Read only token",
      revokedAt: null,
      revokedBy: null,
      roleCode: "viewer",
      status: "active",
      tokenPrefix: "cp_pat_read",
      updatedAt: "2026-06-18T08:00:00Z",
      userDisplayName: "Member User",
      userId: "2",
      username: "member",
  ...
  const tokenTable = page.locator(".console-api-token-table");
  await expect(tokenTable.getByText("cp_pat_read", { exact: true })).toBeVisible();

  const permissionTitle = (permission: string) =>
    zhCN.admin.apiTokens.states.writePermissionDescription.replace("{{permission}}", permission);
  const issueButton = page.getByRole("button", { name: zhCN.admin.apiTokens.actions.issue });
  await expect(issueButton).toBeDisabled();
  await expect(issueButton).toHaveAttribute("title", permissionTitle("api_token:create"));

  const revokeButton = tokenTable
    .getByRole("row")
    .filter({ hasText: "token-readonly" })
    .getByRole("button", {
      name: zhCN.admin.apiTokens.actions.revokeToken.replace("{{id}}", "token-readonly"),
    });
  await expect(revokeButton).toBeDisabled();
  await expect(revokeButton).toHaveAttribute("title", permissionTitle("api_token:revoke"));
  expect(writeRequests).toBe(0);
});
...
test("admin audit logs route renders backend audit records with filters", async ({ page }) => {
  ...
  const auditTable = page.locator(".console-audit-table");
...
test("admin login logs route renders auth.login audit records with local IP filter", async ({
  ...
  const loginTable = page.locator(".console-login-log-table");
...
test("admin API catalog route renders backend route entries with local filters", async ({
  ...
  const apiTable = page.locator(".console-api-table");
...
test("admin API catalog disables sync actions without platform sync grant", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "permission:read", productCode: "console-platform", scope: "platform" }],
  });
  const apiGroups: SystemAPIGroup[] = [
    {
      code: "system",
      count: 1,
      label: "system",
  ...
  let permissionSyncRequests = 0;
  let routeSyncRequests = 0;
  ...
  await page.route("**/api/v1/system/apis", async (route) => {
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({ code: 0, data: apiGroups }),
    });
  });

  await page.goto("/admin/apis");

  const syncRoutesButton = page.getByRole("button", {
    name: zhCN.admin.apis.actions.syncRoutes,
  });
  await expect(syncRoutesButton).toBeDisabled();
  await expect(syncRoutesButton).toHaveAttribute("title", zhCN.admin.apis.actions.syncDisabled);

  const syncPermissionsButton = page.getByRole("button", {
    name: zhCN.admin.apis.actions.syncPermissions,
  });
  await expect(syncPermissionsButton).toBeDisabled();
  await expect(syncPermissionsButton).toHaveAttribute(
    "title",
    zhCN.admin.apis.actions.syncDisabled,
  );
  expect(routeSyncRequests).toBe(0);
  expect(permissionSyncRequests).toBe(0);
});
...
test("admin menu catalog route renders backend-filtered menu groups", async ({ page }) => {
  ...
  const menuCatalog = page.locator(".console-menu-groups");
  await expect(menuCatalog.getByText("workspace", { exact: true })).toBeVisible();
  await expect(menuCatalog.getByText("identity", { exact: true })).toBeVisible();
  await expect(menuCatalog.getByText("integration", { exact: true })).toBeVisible();
...
test("admin system settings route updates backend-editable configuration values", async ({
  ...
  await expect(page.getByText("Console Platform", { exact: true }).first()).toBeVisible();
  ...
  const localizationGroup = page.locator(".console-config-group-card").filter({
...
test("admin system settings route disables updates without config update grant", async ({
  page,
}) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "config:read", productCode: "console-platform", scope: "platform" }],
  });
  let writeRequests = 0;
  ...
  await expect(page.getByText("brand.productName", { exact: true })).toBeVisible();
  const permissionTitle = zhCN.admin.system.states.writePermissionDescription.replace(
    "{{permission}}",
    "config:update",
  );
  const identityGroup = page.locator(".console-config-group-card").filter({
    hasText: "Identity",
  });
  const editButton = identityGroup.getByRole("button", {
    name: zhCN.admin.system.actions.editGroup,
  });
  await expect(editButton).toBeDisabled();
  await expect(editButton).toHaveAttribute("title", permissionTitle);
  expect(writeRequests).toBe(0);
});
...
test("admin media route manages assets through backend media contracts", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken);
  const fileBuffer = Buffer.from("console direct upload");
  ...
  await expect
    ...
    .toBeTruthy();
  await page
    .locator(".console-media-category-button")
  ...
  await expect(
    page.getByRole("dialog", { name: zhCN.admin.media.categories.editTitle }),
  ).toBeHidden();
  ...
  await expect(
    page.locator(".console-media-category-list").getByText("Campaign assets", { exact: true }),
  ).toHaveCount(0);
...
test("admin media route disables writes without media operation grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  const productCode = "console-platform";
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "media:read", productCode, scope: "platform" }],
  });
  const categories: SystemMediaCategory[] = [
    {
      children: [],
      createdAt: "2026-06-18T08:00:00Z",
      id: "10",
  ...
  const denyWrite = async (route: Route) => {
    writeRequests += 1;
    await route.fulfill({
      contentType: "application/json",
      status: 403,
  ...
  await page.route("**/api/v1/system/media/categories/10", denyWrite);
  await page.route(/\/api\/v1\/system\/media\/assets(?:\?.*)?$/, async (route) => {
    if (route.request().method() !== "GET") {
      await denyWrite(route);
      return;
  ...
  await page.route("**/api/v1/system/media/assets/asset-1", denyWrite);
  await page.route("**/api/v1/system/media/assets/upload", denyWrite);
  await page.route("**/api/v1/system/media/assets/import-url", denyWrite);
  ...
  const mediaPermissionTitle = (permission: string) =>
    zhCN.admin.media.states.writePermissionDescription.replace("{{permission}}", permission);
  const createCategoryButton = page.getByRole("button", {
    name: zhCN.admin.media.actions.createCategory,
  });
  await expect(createCategoryButton).toBeDisabled();
  await expect(createCategoryButton).toHaveAttribute("title", mediaPermissionTitle("media:update"));

  const categoryGroup = page
    .locator(".console-media-category-item")
    .filter({ hasText: "Marketing" });
  const editCategoryButton = categoryGroup.getByRole("button", {
    name: zhCN.admin.media.actions.editCategory.replace("{{name}}", "Marketing"),
  });
  await expect(editCategoryButton).toBeDisabled();
  await expect(editCategoryButton).toHaveAttribute("title", mediaPermissionTitle("media:update"));
  const deleteCategoryButton = categoryGroup.getByRole("button", {
    name: zhCN.admin.media.actions.deleteCategory.replace("{{name}}", "Marketing"),
  });
  await expect(deleteCategoryButton).toBeDisabled();
  await expect(deleteCategoryButton).toHaveAttribute("title", mediaPermissionTitle("media:update"));

  const uploadButton = page.getByRole("button", { name: zhCN.admin.media.actions.upload });
  await expect(uploadButton).toBeDisabled();
  await expect(uploadButton).toHaveAttribute("title", mediaPermissionTitle("media:upload"));
  const importButton = page.getByRole("button", { name: zhCN.admin.media.actions.importUrls });
  await expect(importButton).toBeDisabled();
  await expect(importButton).toHaveAttribute("title", mediaPermissionTitle("media:import"));
  await expect(page.getByLabel(zhCN.admin.media.write.import.label)).toBeDisabled();

  const assetRow = page.getByRole("row", { name: /launch-cover\.png/ });
  const renameButton = assetRow.getByRole("button", {
    name: zhCN.admin.media.actions.renameAsset.replace("{{name}}", "launch-cover.png"),
  });
  await expect(renameButton).toBeDisabled();
  await expect(renameButton).toHaveAttribute("title", mediaPermissionTitle("media:update"));
  const downloadButton = assetRow.getByRole("button", {
    name: zhCN.admin.media.actions.downloadAsset.replace("{{name}}", "launch-cover.png"),
  });
  await expect(downloadButton).toBeDisabled();
  await expect(downloadButton).toHaveAttribute("title", mediaPermissionTitle("media:download"));
  const deleteButton = assetRow.getByRole("button", {
    name: zhCN.admin.media.actions.deleteAsset.replace("{{name}}", "launch-cover.png"),
  });
  await expect(deleteButton).toBeDisabled();
  await expect(deleteButton).toHaveAttribute("title", mediaPermissionTitle("media:delete"));
  expect(writeRequests).toBe(0);
...
test("admin media resumable route uploads and aborts backend sessions", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken);
  const fileBuffer = Buffer.from("console resumable upload");
...
test("admin media resumable route disables upload workflow without media upload grant", async ({
  page,
}) => {
  const accessToken = accessTokenWithOrg("1");
  const productCode = "console-platform";
  ...
  const denyUploadWrite = async (route: Route) => {
    writeRequests += 1;
    await route.fulfill({
      contentType: "application/json",
      status: 403,
  ...
  await page.route("**/api/v1/system/media/assets/resumable/check", denyUploadWrite);
  await page.route("**/api/v1/system/media/assets/resumable/chunks", denyUploadWrite);
  await page.route("**/api/v1/system/media/assets/resumable/complete", denyUploadWrite);
  await page.route("**/api/v1/system/media/assets/resumable/abort", denyUploadWrite);
  ...
  const permissionTitle = zhCN.admin.mediaResumable.states.writePermissionDescription.replace(
    "{{permission}}",
    "media:upload",
  );
  const chooseFileButton = page.getByRole("button", {
    name: zhCN.admin.mediaResumable.actions.chooseFile,
  });
  await expect(chooseFileButton).toBeDisabled();
  await expect(chooseFileButton).toHaveAttribute("title", permissionTitle);
  const uploadButton = page.getByRole("button", {
    name: zhCN.admin.mediaResumable.actions.upload,
  });
  await expect(uploadButton).toBeDisabled();
  await expect(uploadButton).toHaveAttribute("title", permissionTitle);
  const abortButton = page.getByRole("button", {
    name: zhCN.admin.mediaResumable.actions.abort,
  });
  await expect(abortButton).toBeDisabled();
  await expect(abortButton).toHaveAttribute("title", permissionTitle);
  await expect(page.getByLabel(zhCN.admin.mediaResumable.a11y.fileInput)).toBeDisabled();
  await expect(page.getByLabel(zhCN.admin.mediaResumable.fields.category)).toBeDisabled();
  expect(writeRequests).toBe(0);
});
...
test("admin traffic hijack route renders probe targets, results, events, and SSE status", async ({
  ...
  await expect(
    page.locator(".console-traffic-target-cell").filter({ hasText: "Public edge" }).first(),
  ).toBeVisible();
...
test("admin traffic hijack route disables writes without mutation grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [
      { code: "traffic_hijack:read", productCode: "console-platform", scope: "platform" },
    ],
  });
  let writeRequests = 0;
  const target: SystemTrafficProbeTarget = {
  ...
  await page.route("**/api/v1/system/traffic-hijack/**", async (route) => {
    const request = route.request();
  ...
  await expect(page.getByLabel(zhCN.admin.trafficHijack.form.fields.name)).toBeDisabled();
  await expect(page.getByLabel(zhCN.admin.trafficHijack.form.fields.url)).toBeDisabled();

  const updatePermissionTitle = zhCN.admin.trafficHijack.states.writePermissionDescription.replace(
    "{{permission}}",
    "traffic_hijack:update",
  );
  const deletePermissionTitle = zhCN.admin.trafficHijack.states.writePermissionDescription.replace(
    "{{permission}}",
    "traffic_hijack:delete",
  );
  const createButton = page.getByRole("button", {
    name: zhCN.admin.trafficHijack.actions.create,
  });
  await expect(createButton).toBeDisabled();
  await expect(createButton).toHaveAttribute("title", updatePermissionTitle);

  const targetRow = page.getByRole("row", { name: /Public edge/ });
  const probeButton = targetRow.getByRole("button", {
    name: zhCN.admin.trafficHijack.actions.probe,
  });
  await expect(probeButton).toBeDisabled();
  await expect(probeButton).toHaveAttribute("title", updatePermissionTitle);
  const editButton = targetRow.getByRole("button", {
    name: zhCN.admin.trafficHijack.actions.edit,
  });
  await expect(editButton).toBeDisabled();
  await expect(editButton).toHaveAttribute("title", updatePermissionTitle);
  const deleteButton = targetRow.getByRole("button", {
    name: zhCN.admin.trafficHijack.actions.delete,
  });
  await expect(deleteButton).toBeDisabled();
  await expect(deleteButton).toHaveAttribute("title", deletePermissionTitle);

  const eventRow = page.getByRole("row", { name: /unexpected status code/ });
  const resolveButton = eventRow.getByRole("button", {
    name: zhCN.admin.trafficHijack.actions.resolve,
  });
  await expect(resolveButton).toBeDisabled();
  await expect(resolveButton).toHaveAttribute("title", updatePermissionTitle);
  expect(writeRequests).toBe(0);
});
...
test("admin versions route covers backend version package workflows", async ({ page }) => {
  ...
  const exportForm = page.locator("form.console-version-export-form");
  ...
  await page
    .locator("form.console-version-import-form")
  ...
  await page.locator(".console-version-table .console-data-table-wrap").evaluate((element) => {
...
test("admin versions route disables package writes without version grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "version:read", productCode: "console-platform", scope: "platform" }],
  });
  const versionItem = {
    apiCount: 2,
    createdAt: "2026-06-18T09:00:00Z",
  ...
  await page.route("**/api/v1/system/versions**", async (route) => {
    ...
    if (url.pathname === "/api/v1/system/versions/100") {
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({
          code: 0,
  ...
  await expect(page.getByRole("row", { name: /June Release/ })).toBeVisible();
  const versionPermissionTitle = (permission: string) =>
    zhCN.admin.versions.states.writePermissionDescription.replace("{{permission}}", permission);
  await expect(
    page.getByRole("button", { name: zhCN.admin.versions.actions.createRelease }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: zhCN.admin.versions.actions.createRelease }),
  ).toHaveAttribute("title", versionPermissionTitle("version:create"));
  await expect(
    page.getByRole("button", { name: zhCN.admin.versions.actions.importVersion }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: zhCN.admin.versions.actions.importVersion }),
  ).toHaveAttribute("title", versionPermissionTitle("version:import"));
  await expect(
    page.getByRole("button", { name: zhCN.admin.versions.actions.deleteSelected }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: zhCN.admin.versions.actions.deleteSelected }),
  ).toHaveAttribute("title", versionPermissionTitle("version:delete"));
  await expect(
    page.getByLabel(
      interpolate(zhCN.admin.versions.a11y.selectVersion, {
        name: "June Release",
  ...
  const downloadButton = juneRow.getByRole("button", {
    exact: true,
    name: zhCN.admin.versions.actions.download,
  });
  await expect(downloadButton).toBeDisabled();
  await expect(downloadButton).toHaveAttribute("title", versionPermissionTitle("version:download"));
  await expect(
    juneRow.getByRole("button", { exact: true, name: zhCN.admin.versions.actions.delete }),
  ).toBeDisabled();
  await expect(
    juneRow.getByRole("button", { exact: true, name: zhCN.admin.versions.actions.delete }),
  ).toHaveAttribute("title", versionPermissionTitle("version:delete"));
...
test("admin dictionaries route manages backend dictionary contracts", async ({ page }) => {
  ...
  const dictionaryGroup = (name: string) =>
    page.locator(".console-dictionary-group").filter({
...
test("admin dictionaries route disables writes without dictionary grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "dictionary:read", productCode: "console-platform", scope: "platform" }],
  });
  let writeRequests = 0;
  const dictionaries: SystemDictionary[] = [
    {
      code: "system.status",
      createdAt: "2026-06-18T08:00:00Z",
      description: "System entity states",
  ...
  await expect(page.getByText("System status", { exact: true })).toBeVisible();
  await page.getByLabel(zhCN.admin.dictionaries.filters.keyword).fill("enabled");
  await expect(page.getByText("Enabled", { exact: true })).toBeVisible();

  const writePermissionTitle = (permission: string) =>
    zhCN.admin.dictionaries.states.writePermissionDescription.replace("{{permission}}", permission);
  const dictionaryGroup = page.locator(".console-dictionary-group").filter({
    has: page.getByRole("heading", { level: 3, name: "System status" }),
  });
  const createButton = page.getByRole("button", { name: zhCN.admin.dictionaries.actions.create });
  await expect(createButton).toBeDisabled();
  await expect(createButton).toHaveAttribute("title", writePermissionTitle("dictionary:create"));
  const editDictionaryButton = dictionaryGroup
    .getByRole("button", {
      name: zhCN.admin.dictionaries.actions.edit,
    })
    .first();
  await expect(editDictionaryButton).toBeDisabled();
  await expect(editDictionaryButton).toHaveAttribute(
    "title",
    writePermissionTitle("dictionary:update"),
  );
  const addItemButton = dictionaryGroup.getByRole("button", {
    name: zhCN.admin.dictionaries.actions.addItem,
  });
  await expect(addItemButton).toBeDisabled();
  await expect(addItemButton).toHaveAttribute("title", writePermissionTitle("dictionary:update"));
  const deleteDictionaryButton = dictionaryGroup
    .getByRole("button", { name: zhCN.admin.dictionaries.actions.delete })
    .last();
  await expect(deleteDictionaryButton).toBeDisabled();
  await expect(deleteDictionaryButton).toHaveAttribute(
    "title",
    writePermissionTitle("dictionary:delete"),
  );
  const itemRow = dictionaryGroup.getByRole("row").filter({ hasText: "Enabled" });
  const editItemButton = itemRow.getByRole("button", {
    name: zhCN.admin.dictionaries.actions.edit,
  });
  await expect(editItemButton).toBeDisabled();
  await expect(editItemButton).toHaveAttribute("title", writePermissionTitle("dictionary:update"));
  const deleteItemButton = itemRow.getByRole("button", {
    name: zhCN.admin.dictionaries.actions.delete,
  });
  await expect(deleteItemButton).toBeDisabled();
  await expect(deleteItemButton).toHaveAttribute(
    "title",
    writePermissionTitle("dictionary:delete"),
  );
  expect(writeRequests).toBe(0);
...
test("admin operation records route disables delete without operation delete grant", async ({
  page,
}) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "operation:read", productCode: "console-platform", scope: "platform" }],
  });
  let writeRequests = 0;
  ...
  await expect(page.getByText("/api/v1/system/apis", { exact: true })).toBeVisible();
  await expect(
    page.getByLabel(zhCN.admin.operationRecords.selection.rowAria.replace("{{id}}", "701")),
  ).toBeDisabled();
  const deleteButton = page.getByRole("button", {
    name: zhCN.admin.operationRecords.actions.deleteSelected,
  });
  await expect(deleteButton).toBeDisabled();
  await expect(deleteButton).toHaveAttribute(
    "title",
    zhCN.admin.operationRecords.states.deletePermissionDescription,
  );
  expect(writeRequests).toBe(0);
});
...
test("admin notification outbox route renders sanitized delivery tasks with retry", async ({
  page,
}) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken);
  const rawToken = "raw-email-verification-token";
  const rawURL = `https://console.example/setup/verify?token=${rawToken}`;
  const protectedRequests: Array<{
    authorization: string | null;
    locale: string | null;
    method: string;
  ...
  type OutboxFixture = IAMNotificationOutboxItem & {
    token?: string;
    url?: string;
  };
  let outboxItems: OutboxFixture[] = [
    {
      attempts: 3,
      createdAt: "2026-06-18T09:00:00Z",
      id: "9901",
      ...
      lastError: "smtp down",
      maxAttempts: 4,
      nextAttemptAt: "2026-06-18T09:05:00Z",
    ...
    },
    {
      ...
      lastError: "",
      maxAttempts: 4,
      nextAttemptAt: "2026-06-18T08:00:00Z",
      recipient: "sent@example.com",
      resourceId: "8802",
      resourceType: "invitation",
      sentAt: "2026-06-18T08:01:00Z",
      status: "sent",
      updatedAt: "2026-06-18T08:01:00Z",
    },
  ];

  await page.route("**/api/v1/iam/notification-outbox**", async (route) => {
    const request = route.request();
    const url = new URL(request.url());
    const method = request.method();
    protectedRequests.push({
      ...
      outboxItems = outboxItems.map((item) =>
        String(item.id) === "9901"
          ? {
              ...item,
              lastError: "",
              sentAt: "2026-06-18T09:06:00Z",
              status: "sent",
              updatedAt: "2026-06-18T09:06:00Z",
            }
          : item,
      );
      await route.fulfill({
        ...
        body: JSON.stringify({
          code: 0,
          data: outboxItems.find((item) => String(item.id) === "9901"),
        }),
      });
      return;
    }
    ...
    const filteredItems = outboxItems.filter(
      (item) =>
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: zhNotificationOutbox.title }),
  ).toBeVisible();
  const failedRow = page.getByRole("row").filter({ hasText: "owner@example.com" });
  await expect(failedRow.getByText(zhNotificationOutbox.status.failed)).toBeVisible();
  await expect(failedRow.getByText("smtp down", { exact: true })).toBeVisible();
  const sentRow = page.getByRole("row").filter({ hasText: "sent@example.com" });
  await expect(sentRow.getByText(zhNotificationOutbox.status.sent)).toBeVisible();
  await expect(
    sentRow.getByRole("button", { name: zhNotificationOutbox.actions.retry }),
  ).toBeDisabled();
  await expect(page.getByText(rawToken, { exact: true })).toHaveCount(0);
  await expect(page.getByText(rawURL, { exact: true })).toHaveCount(0);

  await page.getByLabel(zhNotificationOutbox.filters.status).selectOption("failed");
  await page.getByLabel(zhNotificationOutbox.filters.kind).selectOption("email_verification");
  await page.getByLabel(zhNotificationOutbox.filters.recipient).fill("owner");
  await page.getByLabel(zhNotificationOutbox.filters.pageSize).fill("20");
  await page.getByRole("button", { name: zhNotificationOutbox.actions.search }).click();
  await expect
    .poll(() =>
      protectedRequests.some(
        (request) =>
          request.query.status === "failed" &&
          request.query.kind === "email_verification" &&
          request.query.recipient === "owner" &&
          request.query.pageSize === "20",
      ),
    )
    .toBeTruthy();

  await failedRow.getByRole("button", { name: zhNotificationOutbox.actions.retry }).click();
  await expect(page.getByText(zhNotificationOutbox.retry.successTitle)).toBeVisible();
  expect(protectedRequests).toEqual(
    protectedRequests.map((request) => ({
      authorization: `Bearer ${accessToken}`,
      locale: "zh-CN",
  ...
  expect(new Set(protectedRequests.map((request) => request.path))).toEqual(
    new Set(["/api/v1/iam/notification-outbox", "/api/v1/iam/notification-outbox/9901/retry"]),
  );
});
...
test("admin parameters route manages backend parameter contracts", async ({ page }) => {
  ...
  await expect(
    page.locator(".console-parameter-name").filter({ hasText: "Site title" }),
  ).toBeVisible();
  await expect(page.getByText("site.title", { exact: true })).toBeVisible();
  await expect(page.getByText("Console Platform", { exact: true })).toBeVisible();

  await page.getByRole("button", { name: zhCN.admin.parameters.actions.create }).click();
  const form = page.locator(".console-parameter-form-panel");
...
test("admin parameters route disables writes without parameter grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "parameter:read", productCode: "console-platform", scope: "platform" }],
  });
  ...
  const writePermissionTitle = zhCN.admin.parameters.states.writePermissionDescription.replace(
    "{{permission}}",
    "parameter:create",
  );
  await expect(
    page.getByRole("button", { name: zhCN.admin.parameters.actions.create }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: zhCN.admin.parameters.actions.create }),
  ).toHaveAttribute("title", writePermissionTitle);
  await expect(
    page.getByRole("button", {
      name: interpolate(zhCN.admin.parameters.actions.editFor, { name: "Site title" }),
    }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", {
      name: interpolate(zhCN.admin.parameters.actions.deleteFor, { name: "Site title" }),
    }),
  ).toBeDisabled();
  await expect(
    page.getByLabel(interpolate(zhCN.admin.parameters.selection.rowAria, { id: "301" })),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: zhCN.admin.parameters.actions.deleteSelected }),
  ).toBeDisabled();
  expect(writeRequests).toBe(0);
});

test("admin announcements route manages module announcement contracts", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken);
  const protectedRequests: Array<{
    authorization: string | null;
  ...
  await page.route("**/api/v1/announcements**", async (route) => {
    ...
    if (method === "POST" && url.pathname === "/api/v1/announcements") {
      const input = body as AnnouncementInputBody;
    ...
    if (method === "PATCH") {
      const announcementId = url.pathname.split("/").at(-1);
      const input = body as Partial<AnnouncementInputBody>;
      ...
      announcements = announcements.map((announcement) =>
        String(announcement.id) === announcementId ? updated : announcement,
      );
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({ code: 0, data: updated }),
    ...
    if (method === "POST" && url.pathname.endsWith("/publish")) {
      const announcementId = url.pathname.split("/").at(-2);
      announcements = announcements.map((announcement) =>
        String(announcement.id) === announcementId
          ? {
              ...announcement,
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          code: 0,
          data: announcements.find((announcement) => String(announcement.id) === announcementId),
        }),
      });
      return;
    }
    ...
    if (method === "POST" && url.pathname.endsWith("/archive")) {
      ...
      announcements = announcements.map((announcement) =>
        String(announcement.id) === announcementId
          ? {
              ...announcement,
              archivedAt: "2026-06-22T09:30:00Z",
              status: "archived",
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: zhCN.admin.announcements.title }),
  ).toBeVisible();
  await expect(
    page.locator(".console-announcement-title").filter({ hasText: "Release note" }),
  ).toBeVisible();

  await page.getByRole("button", { name: zhCN.admin.announcements.actions.create }).click();
  const form = page.locator(".console-announcement-form-panel");
  await form.getByLabel(zhCN.admin.announcements.form.title).fill("Maintenance bulletin");
  await form.getByLabel(zhCN.admin.announcements.form.summary).fill("Scheduled maintenance");
  await form.getByLabel(zhCN.admin.announcements.form.content).fill("Maintenance starts at 22:00.");
  await form.getByRole("button", { name: zhCN.admin.announcements.actions.create }).press("Enter");
  await expect(page.getByText(zhCN.admin.announcements.messages.createdTitle)).toBeVisible();
  await expect(
    page.locator(".console-announcement-title").filter({ hasText: "Maintenance bulletin" }),
  ).toBeVisible();

  await page
    .getByRole("button", {
      name: interpolate(zhCN.admin.announcements.actions.editFor, {
        title: "Maintenance bulletin",
      }),
    })
    .press("Enter");
  await form.getByLabel(zhCN.admin.announcements.form.content).fill("Maintenance starts at 23:00.");
  await form.getByRole("button", { name: zhCN.admin.announcements.actions.save }).press("Enter");
  await expect(page.getByText(zhCN.admin.announcements.messages.updatedTitle)).toBeVisible();

  await page
    .getByRole("button", {
      name: interpolate(zhCN.admin.announcements.actions.publishFor, {
        title: "Maintenance bulletin",
      }),
    })
    .press("Enter");
  await expect(page.getByText(zhCN.admin.announcements.messages.publishedTitle)).toBeVisible();
  await expect(
    page
      .locator(".console-announcement-status")
      .getByText(zhCN.admin.announcements.status.published, { exact: true })
      .first(),
  ).toBeVisible();

  await page
    .getByRole("button", {
      name: interpolate(zhCN.admin.announcements.actions.archiveFor, {
        title: "Maintenance bulletin",
      }),
    })
    .press("Enter");
  await expect(page.getByText(zhCN.admin.announcements.messages.archivedTitle)).toBeVisible();

  await page
    .getByRole("button", {
      name: interpolate(zhCN.admin.announcements.actions.deleteFor, {
        title: "Maintenance bulletin",
      }),
    })
    .press("Enter");
  await page
    .getByRole("button", { name: zhCN.admin.announcements.actions.confirmDelete })
    .press("Enter");
  await expect(page.getByText(zhCN.admin.announcements.messages.deletedTitle)).toBeVisible();

  await page.getByLabel(zhCN.admin.announcements.filters.keyword).fill("Release");
  await page.getByLabel(zhCN.admin.announcements.filters.status).selectOption("published");
  await page.getByRole("button", { name: zhCN.admin.announcements.actions.search }).press("Enter");
  await expect
    .poll(() =>
      protectedRequests.some(
        (request) => request.query.keyword === "Release" && request.query.status === "published",
      ),
    )
    .toBeTruthy();
  ...
  expect(new Set(protectedRequests.map((request) => `${request.method} ${request.path}`))).toEqual(
    new Set([
      ...
      "PATCH /api/v1/announcements/503",
      "POST /api/v1/announcements/503/publish",
      "POST /api/v1/announcements/503/archive",
      "DELETE /api/v1/announcements/503",
    ]),
  );
});

test("admin announcements route disables writes without announcement grants", async ({ page }) => {
  const accessToken = accessTokenWithOrg("1");
  await setAuthenticatedSession(page, accessToken, {
    permissions: [{ code: "announcement:read", productCode: "console-platform", scope: "tenant" }],
  });
  ...
  await expect(
    page.locator(".console-announcement-title").filter({ hasText: "Maintenance notice" }),
  ).toBeVisible();

  const permissionTitle = (permission: string) =>
    zhCN.admin.announcements.states.writePermissionDescription.replace(
      "{{permission}}",
      permission,
    );
  const createButton = page.getByRole("button", {
    name: zhCN.admin.announcements.actions.create,
  });
  await expect(createButton).toBeDisabled();
  await expect(createButton).toHaveAttribute("title", permissionTitle("announcement:create"));

  const row = page.getByRole("row", { name: /Maintenance notice/ });
  ...
  await expect(editButton).toBeDisabled();
  await expect(editButton).toHaveAttribute("title", permissionTitle("announcement:update"));
  ...
  await expect(publishButton).toHaveAttribute("title", permissionTitle("announcement:update"));
  const archiveButton = row.getByRole("button", {
    name: interpolate(zhCN.admin.announcements.actions.archiveFor, {
      title: "Maintenance notice",
    }),
  });
  await expect(archiveButton).toBeDisabled();
  await expect(archiveButton).toHaveAttribute("title", permissionTitle("announcement:update"));
...
test("setup route is independent from public and platform console shells", async ({ page }) => {
  ...
  await expect(page.locator(".console-setup-shell")).toBeVisible();
  await expect(page.locator(".console-public-header")).toHaveCount(0);
  await expect(page.locator(".console-platform-sidebar")).toHaveCount(0);
  await expect(
...
test("setup language selection uses canonical locale for following setup requests and default locale", async ({
  page,
}) => {
  await preferZhCN(page);
  const statusLocales: string[] = [];
  ...
  await page.getByLabel(zhCN.setup.language.field).selectOption("en-US");
  await expect(
...
test("setup site step saves only backend-exposed site display fields", async ({ page }) => {
  ...
  await expect(page.getByLabel("Product name")).toHaveValue("Console Platform");
  await page.getByLabel("Product name").fill("Demo Console");
  ...
  expect(saveBody).toMatchObject({
    ...
    test: true,
    values: {
      "brand.productName": "Demo Console",
...
test("setup wizard renders backend test repair hints and env-managed save warnings", async ({
  ...
  expect(testBody).toMatchObject({
    values: {
      ...
      "database.sqlite.path": "data/console.db",
    },
  });
  expect(saveBody).toMatchObject({
    persist: true,
...
test("setup owner step validates password confirmation without submitting it", async ({ page }) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          steps: [
            {
              description: "创建首个平台 owner 账号，并加载默认 IAM 权限策略。",
              fields: [
                {
                  key: "orgCode",
                  label: "组织 Code",
            ...
            {
              description: "验证必要配置、基础数据、权限和运行依赖已经就绪。",
              fields: [],
              groups: [],
              key: "verify.finish",
              order: 90,
  ...
  await expect(page.getByRole("heading", { level: 1, name: "管理员账号" })).toBeVisible();

  await page.getByLabel("组织 Code").fill("default");
  await page.getByLabel("组织名称").fill("Default Organization");
  await page.getByLabel("管理员用户名").fill("owner");
  await page.getByLabel("管理员邮箱").fill("owner@example.com");
  await page.getByLabel("管理员密码", { exact: true }).fill("ConsolePassword123");
  ...
  await page.getByLabel(zhCN.setup.owner.passwordConfirm.label).fill("ConsolePassword123");
  await page.getByRole("button", { name: zhCN.setup.actions.save }).click();
...
test("completed setup redirects setup routes to login when no session exists", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/login$/);
  await expect(page.locator(".console-setup-shell")).toHaveCount(0);
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
});
