...
func TestCopyConfigCoversAllFieldsAndDeepCopiesSlices(t *testing.T) {
	...
	*got.System.MaintenanceCleanupIntervalSeconds = 120
	*got.System.MaintenanceCleanupBatchSize = 25
	...
	*got.WebUI.Enabled = false
	...
	if *src.System.MaintenanceCleanupIntervalSeconds == *got.System.MaintenanceCleanupIntervalSeconds {
		t.Fatal("copyConfig() shares System.MaintenanceCleanupIntervalSeconds pointer with source")
	}
	if *src.System.MaintenanceCleanupBatchSize == *got.System.MaintenanceCleanupBatchSize {
		t.Fatal("copyConfig() shares System.MaintenanceCleanupBatchSize pointer with source")
	}
...
func TestUpdateWithErrorReturnsBeforeReplacingConfig(t *testing.T) {
	t.Parallel()

	src := testCompleteConfig()
	m := &manager{}
	m.config.Store(src)
	updateErr := errors.New("unsupported runtime config path")

	err := m.UpdateWithError(func(cfg *Config) error {
		cfg.Server.Port = 9091
		return updateErr
	})

	if !errors.Is(err, updateErr) {
		t.Fatalf("UpdateWithError() error = %v, want %v", err, updateErr)
	}
	if got := m.Get().Server.Port; got != src.Server.Port {
		t.Fatalf("UpdateWithError() replaced config after update error, got port %d want %d", got, src.Server.Port)
	}
}
...
func TestEnvNamesUseDynamicAppPrefix(t *testing.T) {
	if EnvPrefix() != "APP" {
		t.Fatalf("EnvPrefix() = %q, want APP", EnvPrefix())
	}
	dbHostEnvName := taggedEnvName(t, DatabasePostgresConfig{}, "Host")
	if EnvPrefixJoin(dbHostEnvName) != "APP_DB_POSTGRES_HOST" {
		t.Fatalf("EnvPrefixJoin(%q) = %q, want APP_DB_POSTGRES_HOST", dbHostEnvName, EnvPrefixJoin(dbHostEnvName))
	}
	if EnvConfigPathName() != "CONFIG_PATH" {
		t.Fatalf("EnvConfigPathName() = %q, want CONFIG_PATH", EnvConfigPathName())
	}
}
...
func TestOverrideWithEnvUsesEnvnameTagsForNonDatabaseConfigs(t *testing.T) {
	...
	setTaggedEnv(t, SystemConfig{}, "MaintenanceCleanupIntervalSeconds", "120")
	setTaggedEnv(t, SystemConfig{}, "MaintenanceCleanupBatchSize", "25")
	...
	if cfg.System.MaintenanceCleanupIntervalSecondsValue() != 120 || cfg.System.MaintenanceCleanupBatchSizeValue() != 25 {
		t.Fatalf("System maintenance override mismatch: %#v", cfg.System)
	}
}
...
func TestLoadEnvSkipsMissingDotEnv(t *testing.T) {
	tempDir := t.TempDir()
	oldWD, err := os.Getwd()
	if err != nil {
		t.Fatalf("get wd: %v", err)
	...
	if err := LoadEnv(); err != nil {
		t.Fatalf("LoadEnv() should skip missing .env, got %v", err)
	}
}

func TestManagerLoadReturnsDotEnvParseError(t *testing.T) {
	tempDir := t.TempDir()
	configPath := filepath.Join(tempDir, "config.yaml")
	dotEnvPath := filepath.Join(tempDir, EnvFilePath)
	writeTestConfig(t, configPath)
	if err := os.WriteFile(dotEnvPath, []byte("APP_SERVER_PORT='unterminated\n"), 0o600); err != nil {
		t.Fatalf("write .env: %v", err)
	}
	...
	m := NewManager()
	err = m.Load(configPath)
	if err == nil || !strings.Contains(err.Error(), "failed to load .env file") {
		t.Fatalf("Load() error = %v, want .env parse context", err)
	}
}
...
func testCompleteConfig() *Config {
	return &Config{
		...
		System: SystemConfig{
			...
			MaintenanceCleanupIntervalSeconds: intPtr(60),
			MaintenanceCleanupBatchSize:       intPtr(100),
		},
		CORS: CORSConfig{
			Enabled:          true,
...
func TestManagerLoadUsesOnlyExplicitConfigFile(t *testing.T) {
	...
	writeTestConfig(t, configPath)
	exampleConfigDir := filepath.Join(root, "_examples", "other-service", "demo")
	if err := os.MkdirAll(exampleConfigDir, 0o700); err != nil {
		t.Fatalf("mkdir example config dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(exampleConfigDir, "config.yaml"), []byte("server:\n  port: 1\n"), 0o600); err != nil {
		t.Fatalf("write unrelated example config: %v", err)
	}
