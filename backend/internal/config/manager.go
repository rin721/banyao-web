...
type Manager interface {
	...
	// UpdateWithError 原子地更新配置,并允许更新函数在持久化前返回错误。
	// 参数:
	//   fn: 更新函数,接收配置副本并修改,可返回路径映射或业务校验错误
	// 返回:
	//   error: 更新函数、验证或持久化失败时的错误
	UpdateWithError(fn func(*Config) error, options ...UpdateOption) error
...
func (m *manager) Update(fn func(*Config), options ...UpdateOption) error {
	return m.UpdateWithError(func(cfg *Config) error {
		fn(cfg)
		return nil
	}, options...)
}

// UpdateWithError 原子地更新配置,并在更新函数返回错误时停止验证、持久化和替换。
func (m *manager) UpdateWithError(fn func(*Config) error, options ...UpdateOption) error {
...
func (m *manager) copyConfig(src *Config) *Config {
	...
	dst.I18n.Resources = copyStringMap(src.I18n.Resources)
...
func (m *manager) handleConfigChange(e configloader.Event) {
	...
	if err := LoadEnv(); err != nil {
		if m.log != nil {
			m.log.Error("failed to load .env file, keeping current config", "error", err)
		}
		return
	}
