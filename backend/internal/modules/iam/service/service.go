...
const (
	...
	DefaultNotificationRetryInterval    = time.Minute
	DefaultNotificationRetryMaxAttempts = 5
)
...
// Principal 是跨层认证主体契约的 IAM 本地别名，真实定义位于 types/auth。
type Principal = authtypes.Principal

// PermissionContext 是跨层权限判断上下文的 IAM 本地别名，真实定义位于 types/auth。
type PermissionContext = authtypes.PermissionContext
...
// PermissionGrant 是当前会话可用于前端动作显隐的授权快照。
// 后端真实鉴权仍以 Authorize 和路由 contract 为准。
...
// RevokeSessionInput 描述撤销组织会话的请求。
...
// NotificationOutboxFilter 描述 IAM 通知 outbox 管理端查询条件。
// 出于凭据安全考虑，查询响应只返回脱敏后的任务状态，不返回 token 或完整链接。
type NotificationOutboxFilter struct {
	Status    string
	Kind      string
	Recipient string
	Page      int
...
// NotificationOutboxView 是 IAM 通知 outbox 的安全展示模型。
type NotificationOutboxView struct {
	ID            int64      `json:"id,string"`
...
// NotificationOutboxPage 是 IAM 通知 outbox 列表分页响应。
type NotificationOutboxPage struct {
	Items         []NotificationOutboxView `json:"items"`
	Page          int                      `json:"page"`
	PageSize      int                      `json:"pageSize"`
	Total         int64                    `json:"total"`
	StorageStatus string                   `json:"storageStatus"`
...
// RetryNotificationOutboxInput 描述管理员手动重试通知 outbox 任务的请求。
...
type NotificationOutboxDispatchInput struct {
	Limit int
}

type NotificationOutboxDispatchResult struct {
	Scanned  int
	Sent     int
	Failed   int
	Deferred int
...
// WarningLogger 是 IAM service 内部 best-effort 缓存路径使用的最小日志端口。
...
type Repository interface {
	...
	ListUserRoleCodes(context.Context, int64, int64) ([]string, error)
	...
	FindPasswordResetByID(context.Context, int64) (*model.PasswordReset, error)
	...
	FindEmailVerificationByID(context.Context, int64) (*model.EmailVerification, error)
	...
	CreateNotificationOutbox(context.Context, *model.NotificationOutbox) error
	FindNotificationOutboxByID(context.Context, int64) (*model.NotificationOutbox, error)
	ListDueNotificationOutbox(context.Context, time.Time, int) ([]model.NotificationOutbox, error)
	ListNotificationOutbox(context.Context, NotificationOutboxFilter) ([]model.NotificationOutbox, int64, error)
	SaveNotificationOutbox(context.Context, *model.NotificationOutbox) error
...
type Service interface {
	...
	RevokeSession(context.Context, RevokeSessionInput) error
	...
	DispatchNotificationOutbox(context.Context, NotificationOutboxDispatchInput) (NotificationOutboxDispatchResult, error)
	ListNotificationOutbox(context.Context, Principal, NotificationOutboxFilter) (NotificationOutboxPage, error)
	RetryNotificationOutbox(context.Context, RetryNotificationOutboxInput) (NotificationOutboxView, error)
}
...
// WithLogger 注入 IAM 降级缓存和后台辅助路径的最小日志端口。
func WithLogger(logger WarningLogger) Option {
	return func(s *service) {
		s.logger = logger
	}
}
...
func New(repo Repository, crypto PasswordCrypto, tokens TokenManager, authz AuthorizerEnforcer, ids IDGenerator, totp TOTPProvider, cfg Config, notifier Notifier, options ...Option) Service {
	...
	if cfg.NotificationRetryInterval <= 0 {
		cfg.NotificationRetryInterval = DefaultNotificationRetryInterval
	}
	if cfg.NotificationRetryMaxAttempts <= 0 {
		cfg.NotificationRetryMaxAttempts = DefaultNotificationRetryMaxAttempts
	}
	...
	svc := &service{
		...
		notificationRetryInterval:    cfg.NotificationRetryInterval,
		notificationRetryMaxAttempts: cfg.NotificationRetryMaxAttempts,
...
func (s *service) ReloadNotificationRuntime(cfg NotificationRuntimeConfig) {
	...
	retryInterval := cfg.NotificationRetryInterval
	if retryInterval <= 0 {
		retryInterval = DefaultNotificationRetryInterval
	}
	maxAttempts := cfg.NotificationRetryMaxAttempts
	if maxAttempts <= 0 {
		maxAttempts = DefaultNotificationRetryMaxAttempts
	}
	s.notificationMu.Lock()
	s.notificationDriver = driver
	s.notificationPublicURL = cfg.PublicBaseURL
	s.notificationRetryInterval = retryInterval
	s.notificationRetryMaxAttempts = maxAttempts
...
// 该入口允许复用已有组织或用户，适合部署脚本反复执行；成功后会重载授权策略，失败则向调用方返回错误。
...
func (s *service) signupWithEmailVerification(ctx context.Context, input SignupInput) (SignupResult, error) {
	...
	err = s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		...
		outbox = s.newNotificationOutbox(model.NotificationOutboxKindEmailVerification, input.Email, rawToken, adminPath, model.NotificationOutboxResourceEmailVerification, verification.ID, now)
		if err := repo.CreateNotificationOutbox(txCtx, outbox); err != nil {
			return err
		}
		return nil
	})
	...
	if err := s.deliverNotificationOutbox(ctx, outbox); err != nil {
...
func (s *service) Login(ctx context.Context, input LoginInput) (TokenPair, error) {
	...
	var pair TokenPair
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		issued, err := s.createSessionAndTokensWithRepo(txCtx, repo, user, org.ID, input.UserAgent, input.IPAddress, input.ProductCode, input.ClientType)
		if err != nil {
			return err
		}
		...
		if err := s.audit(txCtx, repo, &org.ID, &user.ID, "auth.login", "session", "", input.IPAddress, input.UserAgent, nil, SessionContext{ProductCode: input.ProductCode, ClientType: input.ClientType}); err != nil {
			return err
		}
		pair = issued
		return nil
...
func (s *service) Refresh(ctx context.Context, input RefreshInput) (TokenPair, error) {
	...
	out := tokenPair(pair, session)
	permissions, err := s.sessionPermissionGrants(ctx, s.repo, session.UserID, session.OrgID, sessionCtx.ProductCode)
	if err != nil {
		return TokenPair{}, err
	}
...
// 组织、成员关系、内置角色权限和审计记录在同一事务中写入，成功后必须重载授权策略。
...
func (s *service) UpdateOrganization(ctx context.Context, input UpdateOrganizationInput) (*model.Organization, error) {
	...
	var org *model.Organization
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		found, err := repo.FindOrganizationByID(txCtx, input.OrgID)
		if err != nil {
			return ErrNotFound
		}
		found.Name = name
		if err := repo.SaveOrganization(txCtx, found); err != nil {
			return err
		}
		if err := s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "org.update", "organization", strconv.FormatInt(found.ID, 10), input.IPAddress, input.UserAgent, map[string]any{"name": name}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType}); err != nil {
			return err
		}
		org = found
		return nil
...
func (s *service) InviteUser(ctx context.Context, input InviteUserInput) (NotificationDelivery, error) {
	...
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		...
		if err := repo.CreateInvitation(txCtx, invitation); err != nil {
			return err
		}
		outbox = s.newNotificationOutbox(model.NotificationOutboxKindInvitation, email, raw, adminPath, model.NotificationOutboxResourceInvitation, invitation.ID, now)
		if err := repo.CreateNotificationOutbox(txCtx, outbox); err != nil {
			return err
		}
		return s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "user.invite", "invitation", strconv.FormatInt(invitation.ID, 10), input.IPAddress, input.UserAgent, map[string]any{"email": email}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType})
	}); err != nil {
		return NotificationDelivery{}, err
	}
	if err := s.deliverNotificationOutbox(ctx, outbox); err != nil {
...
func (s *service) RevokeInvitation(ctx context.Context, p Principal, invitationID int64, userAgent, ip string) error {
	return s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		invitation, err := repo.FindInvitationByID(txCtx, invitationID)
		if err != nil {
			return ErrNotFound
		}
		...
		return s.audit(txCtx, repo, &p.OrgID, &p.UserID, "invite.revoke", "invitation", strconv.FormatInt(invitation.ID, 10), ip, userAgent, map[string]any{"email": invitation.Email}, SessionContext{ProductCode: p.ProductCode, ClientType: p.ClientType})
	})
}
...
func (s *service) ForgotPassword(ctx context.Context, input ForgotPasswordInput) (NotificationDelivery, error) {
	...
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		...
		if err := repo.CreatePasswordReset(txCtx, reset); err != nil {
			return err
		}
		outbox = s.newNotificationOutbox(model.NotificationOutboxKindPasswordReset, user.Email, raw, adminPath, model.NotificationOutboxResourcePasswordReset, reset.ID, now)
		if err := repo.CreateNotificationOutbox(txCtx, outbox); err != nil {
			return err
		}
		return s.audit(txCtx, repo, nil, &user.ID, "password.forgot", "password_reset", strconv.FormatInt(reset.ID, 10), input.IPAddress, input.UserAgent, nil)
	}); err != nil {
		return NotificationDelivery{}, err
	}
	if err := s.deliverNotificationOutbox(ctx, outbox); err != nil {
...
func (s *service) ResetPassword(ctx context.Context, input ResetPasswordInput) error {
	...
	return s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		...
		reset.UsedAt = &now
		if err := repo.SavePasswordReset(txCtx, reset); err != nil {
			return err
		}
		sessions, err := repo.ListSessionsByUser(txCtx, user.ID)
		if err != nil {
			return err
		}
		for i := range sessions {
			...
			if err := repo.SaveSession(txCtx, &sessions[i]); err != nil {
				return err
			}
		}
		return s.audit(txCtx, repo, nil, &user.ID, "password.reset", "password_reset", strconv.FormatInt(reset.ID, 10), input.IPAddress, input.UserAgent, nil)
	})
}
...
func (s *service) SetupMFA(ctx context.Context, p Principal) (string, string, error) {
	...
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		now := s.now()
		factor, err := repo.FindActiveMFAFactor(txCtx, user.ID)
		...
		if err == nil {
			...
			if err := repo.SaveMFAFactor(txCtx, factor); err != nil {
				return err
			}
			return s.audit(txCtx, repo, &p.OrgID, &p.UserID, "mfa.setup", "mfa_factor", strconv.FormatInt(factor.ID, 10), "", "", nil, SessionContext{ProductCode: p.ProductCode, ClientType: p.ClientType})
		}
		...
		if err := repo.CreateMFAFactor(txCtx, factor); err != nil {
			return err
		}
		return s.audit(txCtx, repo, &p.OrgID, &p.UserID, "mfa.setup", "mfa_factor", strconv.FormatInt(factor.ID, 10), "", "", nil, SessionContext{ProductCode: p.ProductCode, ClientType: p.ClientType})
	}); err != nil {
...
func (s *service) VerifyMFA(ctx context.Context, p Principal, code string) error {
	...
	return s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		user, err := repo.FindUserByID(txCtx, p.UserID)
		if err != nil {
			return err
		}
		now := s.now()
		...
		factor, err := repo.FindActiveMFAFactor(txCtx, p.UserID)
		if err != nil {
			return err
		}
		...
		return s.audit(txCtx, repo, &p.OrgID, &p.UserID, "mfa.verify", "mfa_factor", "", "", "", nil, SessionContext{ProductCode: p.ProductCode, ClientType: p.ClientType})
	})
}
...
func (s *service) ListUsers(ctx context.Context, p Principal, filter UserListFilter) (OrganizationUserPage, error) {
	...
	for _, membership := range memberships {
		...
		roles, err := s.authz.GetRolesForUser(ctx, userSubject(user.ID), strconv.FormatInt(p.OrgID, 10))
		if err != nil {
			return OrganizationUserPage{}, err
		}
...
func (s *service) UpdateUser(ctx context.Context, input UpdateUserInput) (*OrganizationUser, error) {
	...
	var user *model.User
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		foundMembership, err := repo.FindMembershipAnyStatus(txCtx, input.Principal.OrgID, input.UserID)
		if err != nil {
			return ErrNotFound
		}
		foundUser, err := repo.FindUserByID(txCtx, input.UserID)
		if err != nil {
			return ErrNotFound
		}
		if input.Status != nil || input.HasRoles {
			currentRoles, err := repo.ListUserRoleCodes(txCtx, input.Principal.OrgID, input.UserID)
			if err != nil {
				return err
			}
			if rolesContain(currentRoles, model.RolePlatformOwner) {
				return ErrForbidden
			}
		}
		if input.Status != nil {
			...
			case model.StatusActive, model.StatusDisabled:
				foundMembership.Status = status
				if err := repo.SaveMembership(txCtx, foundMembership); err != nil {
					return err
				}
				if err := s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "user.update", "membership", strconv.FormatInt(foundMembership.ID, 10), input.IPAddress, input.UserAgent, map[string]any{"userId": input.UserID, "status": status}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType}); err != nil {
					return err
				}
			default:
		...
		if input.HasRoles {
			for _, roleCode := range input.Roles {
				...
				if _, err := repo.FindRole(txCtx, input.Principal.OrgID, normalizeCode(roleCode)); err != nil {
					return ErrNotFound
				}
			}
			if err := repo.DeleteCasbinRules(txCtx, "g", userSubject(input.UserID), "", strconv.FormatInt(input.Principal.OrgID, 10)); err != nil {
				return err
			}
			for _, roleCode := range input.Roles {
				if err := s.addUserRole(txCtx, repo, input.UserID, input.Principal.OrgID, roleCode); err != nil {
					return err
				}
			}
			if err := s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "user.roles.update", "user", strconv.FormatInt(input.UserID, 10), input.IPAddress, input.UserAgent, map[string]any{"roles": input.Roles}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType}); err != nil {
				return err
			}
		}
	...
	roles, err := s.authz.GetRolesForUser(ctx, userSubject(user.ID), strconv.FormatInt(input.Principal.OrgID, 10))
	if err != nil {
		return nil, err
	}
	s.bumpOrganizationCaches(ctx, input.Principal.ProductCode, input.Principal.OrgID)
...
func (s *service) CreateAPIToken(ctx context.Context, input CreateAPITokenInput) (CreateAPITokenResult, error) {
	...
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		if err := repo.CreateAPIToken(txCtx, apiToken); err != nil {
			return err
		}
...
// ListNotificationOutbox 返回 IAM 通知 outbox 的脱敏管理视图。
func (s *service) ListNotificationOutbox(ctx context.Context, _ Principal, filter NotificationOutboxFilter) (NotificationOutboxPage, error) {
	filter.Status = normalizeCode(filter.Status)
	switch filter.Status {
	case "", model.NotificationOutboxStatusPending, model.NotificationOutboxStatusSent, model.NotificationOutboxStatusFailed:
	default:
		return NotificationOutboxPage{}, ErrInvalidInput
	}
	filter.Kind = normalizeCode(filter.Kind)
	switch filter.Kind {
	case "", model.NotificationOutboxKindInvitation, model.NotificationOutboxKindPasswordReset, model.NotificationOutboxKindEmailVerification:
	default:
		return NotificationOutboxPage{}, ErrInvalidInput
	}
	filter.Page, filter.PageSize = normalizeListPage(filter.Page, filter.PageSize)
	filter.Recipient = strings.TrimSpace(filter.Recipient)
	items, total, err := s.repo.ListNotificationOutbox(ctx, filter)
	if err != nil {
		return NotificationOutboxPage{}, err
	}
	views := make([]NotificationOutboxView, 0, len(items))
	for _, item := range items {
		views = append(views, notificationOutboxView(item))
	}
	return NotificationOutboxPage{Items: views, Page: filter.Page, PageSize: filter.PageSize, Total: total, StorageStatus: "persisted"}, nil
}

// RetryNotificationOutbox 由平台管理员手动触发单条通知 outbox 立即重试。
// 方法只返回脱敏视图；明文 token 和链接仅在 service 内部用于投递。
func (s *service) RetryNotificationOutbox(ctx context.Context, input RetryNotificationOutboxInput) (NotificationOutboxView, error) {
	if input.OutboxID <= 0 {
		return NotificationOutboxView{}, ErrInvalidInput
	}
	var outbox *model.NotificationOutbox
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		found, err := repo.FindNotificationOutboxByID(txCtx, input.OutboxID)
		if err != nil {
			return err
		}
		if found.Status == model.NotificationOutboxStatusSent {
			return ErrInvalidInput
		}
		_, maxAttempts := s.currentNotificationRetryPolicy()
		if found.MaxAttempts <= 0 {
			found.MaxAttempts = maxAttempts
		}
		if found.Attempts >= found.MaxAttempts {
			found.MaxAttempts = found.Attempts + 1
		}
		now := s.now()
		found.Status = model.NotificationOutboxStatusPending
		found.NextAttemptAt = now
		found.UpdatedAt = now
		if err := repo.SaveNotificationOutbox(txCtx, found); err != nil {
			return err
		}
		...
		if err := s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "notification.retry", "notification_outbox", strconv.FormatInt(found.ID, 10), input.IPAddress, input.UserAgent, metadata, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType}); err != nil {
			return err
		}
		outbox = found
		return nil
	}); err != nil {
		return NotificationOutboxView{}, err
	}
	if err := s.deliverNotificationOutbox(ctx, outbox); err != nil {
		return NotificationOutboxView{}, fmt.Errorf("%w: %v", ErrNotificationDelivery, err)
	}
	return notificationOutboxView(*outbox), nil
}
...
func (s *service) RevokeAPIToken(ctx context.Context, input RevokeAPITokenInput) error {
	...
	return s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		apiToken, err := repo.FindAPITokenByID(txCtx, input.TokenID)
		if err != nil {
			return ErrNotFound
		}
		...
		if err := repo.SaveAPIToken(txCtx, apiToken); err != nil {
			return err
		}
...
// ListRoles 返回当前组织角色，并补齐每个角色的权限列表。
func (s *service) ListRoles(ctx context.Context, p Principal) ([]model.Role, error) {
	...
	for i := range roles {
		if err := s.hydrateRolePermissions(ctx, &roles[i]); err != nil {
			return nil, err
		}
	}
...
// 自定义角色只能分配当前产品已登记的租户级权限，避免把无效或平台级权限静默写成错误策略。
func (s *service) CreateRole(ctx context.Context, input CreateRoleInput) (*model.Role, error) {
	...
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		grants, err := s.assignableRolePermissions(txCtx, repo, input.Principal.ProductCode, input.Permissions)
		if err != nil {
			return err
		}
		...
		for _, grant := range grants {
			if err := s.addPolicy(txCtx, repo, input.Principal.OrgID, code, input.Principal.ProductCode, model.PermissionScopeTenant, grant.object, grant.action); err != nil {
				return err
			}
		}
		return s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "role.create", "role", strconv.FormatInt(role.ID, 10), "", "", map[string]any{"permissions": rolePermissionCodes(grants)}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType})
	}); err != nil {
		return nil, err
	}
	if err := s.LoadPolicies(ctx); err != nil {
	...
	if err := s.hydrateRolePermissions(ctx, role); err != nil {
		return nil, err
	}
	s.bumpOrganizationCaches(ctx, input.Principal.ProductCode, input.Principal.OrgID)
	return role, nil
...
func (s *service) UpdateRole(ctx context.Context, input UpdateRoleInput) (*model.Role, error) {
	var role *model.Role
	if err := s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		found, err := repo.FindRoleByID(txCtx, input.RoleID)
		if err != nil {
			return ErrNotFound
		}
		if found.OrgID != input.Principal.OrgID {
			return ErrForbidden
		}
		if found.System {
			return ErrForbidden
		...
		if err := repo.SaveRole(txCtx, found); err != nil {
			return err
		}
		if input.HasPermissions {
			...
			if err := repo.DeleteCasbinRules(txCtx, "p", roleSubject(found.Code), strconv.FormatInt(input.Principal.OrgID, 10)); err != nil {
				return err
			}
			for _, grant := range grants {
				if err := s.addPolicy(txCtx, repo, input.Principal.OrgID, found.Code, input.Principal.ProductCode, model.PermissionScopeTenant, grant.object, grant.action); err != nil {
					return err
				}
			}
			input.Permissions = rolePermissionCodes(grants)
		}
		if err := s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "role.update", "role", strconv.FormatInt(found.ID, 10), input.IPAddress, input.UserAgent, map[string]any{"permissions": input.Permissions}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType}); err != nil {
			return err
		}
		role = found
		return nil
...
// 会话状态和审计记录必须共事务提交，避免后台显示撤销成功但缺少安全操作记录。
func (s *service) RevokeSession(ctx context.Context, input RevokeSessionInput) error {
	if input.SessionID <= 0 {
		return ErrInvalidInput
	}
	return s.repo.WithTx(ctx, func(txCtx context.Context, repo Repository) error {
		session, err := repo.FindSessionByID(txCtx, input.SessionID)
		if err != nil {
			return ErrNotFound
		}
		if session.OrgID != input.Principal.OrgID {
			return ErrForbidden
		}
		if session.RevokedAt != nil {
			return nil
		...
		now := s.now()
		session.RevokedAt = &now
		if err := repo.SaveSession(txCtx, session); err != nil {
			return err
		}
		return s.audit(txCtx, repo, &input.Principal.OrgID, &input.Principal.UserID, "session.revoke", "session", strconv.FormatInt(session.ID, 10), input.IPAddress, input.UserAgent, map[string]any{"userId": session.UserID}, SessionContext{ProductCode: input.Principal.ProductCode, ClientType: input.Principal.ClientType})
	})
}
...
func (s *service) createSessionAndTokensWithRepo(ctx context.Context, repo Repository, user *model.User, orgID int64, userAgent, ip, productCode, clientType string) (TokenPair, error) {
	...
	out := tokenPair(pair, session)
	permissions, err := s.sessionPermissionGrants(ctx, repo, user.ID, orgID, sessionCtx.ProductCode)
	if err != nil {
		return TokenPair{}, err
	}
	out.Permissions = permissions
...
// sessionPermissionGrants 汇总当前用户在组织内通过角色获得的权限快照。
// 它只用于客户端动作显隐，不能替代服务端 Authorize。
func (s *service) sessionPermissionGrants(ctx context.Context, repo Repository, userID, orgID int64, productCode string) ([]PermissionGrant, error) {
	roles, err := repo.ListUserRoleCodes(ctx, orgID, userID)
	if err != nil {
		return nil, err
	}
	...
	for _, roleCode := range roles {
		permissions, err := repo.ListRolePermissions(ctx, orgID, roleSubject(roleCode))
		if err != nil {
			return nil, err
		}
		for _, permission := range permissions {
			grantProduct := normalizeProductCode(permission.ProductCode)
			if grantProduct != productCode {
				continue
			}
			grantScope := normalizePermissionScope(permission.Scope)
			code := strings.TrimSpace(permission.Code)
			if code == "" || !validPermissionScope(grantScope) {
				continue
			}
			key := grantProduct + "\x00" + grantScope + "\x00" + code
			grants[key] = PermissionGrant{ProductCode: grantProduct, Scope: grantScope, Code: code}
		}
	}
...
func (s *service) cacheGet(ctx context.Context, key string, ttl time.Duration, out any) bool {
	...
	if err != nil {
		s.warnCacheError("iam cache read failed", key, err)
		return false
	}
	return ok
}
...
func (s *service) cacheSet(ctx context.Context, key string, ttl time.Duration, value any) {
	...
	if err := s.cache.SetJSON(ctx, key, value, ttl); err != nil {
		s.warnCacheError("iam cache write failed", key, err)
	}
}
...
func (s *service) cacheEpoch(ctx context.Context, key string) string {
	...
	if err != nil {
		s.warnCacheError("iam cache epoch read failed", key, err)
		return "0"
	}
	if !ok {
		return "0"
...
func (s *service) bumpCacheEpoch(ctx context.Context, key string) {
	...
	if _, err := s.cache.Incr(ctx, key, 0); err != nil {
		s.warnCacheError("iam cache epoch bump failed", key, err)
	}
}

func (s *service) warnCacheError(message string, key string, err error) {
	if s.logger == nil || err == nil {
		return
	}
	s.logger.Warn(message, "key", key, "error", err)
}
...
func (s *service) organizationUsersCacheKey(ctx context.Context, p Principal, filter UserListFilter) string {
	...
	return s.cacheKey(cacheScopeOrgUsers, sessionCtx.ProductCode, orgID, s.cacheEpoch(ctx, epochKey), s.hashCacheValue(cacheScopeOrgUsers, filter))
}
...
func (s *service) hashCacheValue(scope string, value any) string {
	raw, err := json.Marshal(value)
	if err != nil {
		s.warnCacheError("iam cache key hash failed", scope, err)
...
// assignableRolePermissions 校验自定义角色权限。
// 角色管理接口只允许写入当前产品已登记的租户级权限；平台级权限只能通过 platform_owner 等系统角色授予。
func (s *service) assignableRolePermissions(ctx context.Context, repo Repository, productCode string, permissions []string) ([]rolePermissionGrant, error) {
	normalizedProductCode := normalizeProductCode(firstNonEmpty(productCode, s.cfg.DefaultProductCode))
	...
	for _, raw := range permissions {
		...
		obj, act := permissionObjectAction(code)
		if obj == "" || act == "" {
			return nil, ErrInvalidInput
		}
		...
		seen[code] = struct{}{}
		if _, err := repo.FindPermission(ctx, normalizedProductCode, model.PermissionScopeTenant, code); err != nil {
			if errors.Is(err, ErrNotFound) {
				return nil, ErrInvalidInput
			}
			return nil, err
		}
		grants = append(grants, rolePermissionGrant{code: code, object: obj, action: act})
	}
	return grants, nil
}

func rolePermissionCodes(grants []rolePermissionGrant) []string {
	codes := make([]string, 0, len(grants))
	for _, grant := range grants {
		codes = append(codes, grant.code)
	}
	return codes
}
...
// audit 写入 IAM 审计日志。
// metadata 序列化失败必须返回给调用方，避免审计上下文被静默丢弃。
func (s *service) audit(ctx context.Context, repo Repository, orgID, userID *int64, action, resource, resourceID, ip, userAgent string, metadata map[string]any, contexts ...SessionContext) error {
	...
	if err != nil {
		return fmt.Errorf("marshal iam audit metadata: %w", err)
	}
...
func (s *service) issueAPITokenSecret() (string, string, string, error) {
	...
	value := "pat_" + base64.RawURLEncoding.EncodeToString(raw)
...
// DispatchNotificationOutbox 投递到期的 IAM 通知 outbox 任务。
//
// 外部通知属于本地事务之后的副作用：单条失败会写回 outbox 状态并继续处理同批其它任务，
// 最终把合并错误返回给应用层调度器记录，避免底层吞掉失败原因。
func (s *service) DispatchNotificationOutbox(ctx context.Context, input NotificationOutboxDispatchInput) (NotificationOutboxDispatchResult, error) {
	if input.Limit <= 0 || input.Limit > 100 {
		input.Limit = 20
	}
	items, err := s.repo.ListDueNotificationOutbox(ctx, s.now(), input.Limit)
	if err != nil {
		return NotificationOutboxDispatchResult{}, err
	}
	result := NotificationOutboxDispatchResult{Scanned: len(items)}
	var joined error
	for index := range items {
		item := items[index]
		err := s.deliverNotificationOutbox(ctx, &item)
		switch {
		case err != nil:
			if item.Status == model.NotificationOutboxStatusFailed {
				result.Failed++
			} else {
				result.Deferred++
			}
			joined = errors.Join(joined, err)
		case item.Status == model.NotificationOutboxStatusSent:
			result.Sent++
		case item.Status == model.NotificationOutboxStatusFailed:
			result.Failed++
		default:
			result.Deferred++
		}
...
func notificationOutboxView(outbox model.NotificationOutbox) NotificationOutboxView {
	return NotificationOutboxView{
		ID:            outbox.ID,
		Kind:          outbox.Kind,
...
func (s *service) newNotificationOutbox(kind, recipient, token, adminPath, resourceType string, resourceID int64, now time.Time) *model.NotificationOutbox {
	_, maxAttempts := s.currentNotificationRetryPolicy()
	return &model.NotificationOutbox{
		...
		Token:         token,
		URL:           s.notificationURL(adminPath),
...
func (s *service) deliverNotificationOutbox(ctx context.Context, outbox *model.NotificationOutbox) error {
	if outbox == nil || outbox.Status != model.NotificationOutboxStatusPending {
		return nil
	}
	ok, err := s.notificationResourcePending(ctx, outbox)
	if err != nil {
		return err
	}
	if !ok {
		return s.markNotificationOutboxFailure(ctx, outbox, errors.New("notification resource is no longer pending"), true)
	}
	if err := s.sendNotificationOutboxNotice(ctx, outbox); err != nil {
		if saveErr := s.markNotificationOutboxFailure(ctx, outbox, err, false); saveErr != nil {
			return errors.Join(err, saveErr)
		}
		return err
	}
	return s.markNotificationOutboxSent(ctx, outbox)
}

func (s *service) notificationResourcePending(ctx context.Context, outbox *model.NotificationOutbox) (bool, error) {
	now := s.now()
	switch outbox.ResourceType {
	case model.NotificationOutboxResourceInvitation:
		invitation, err := s.repo.FindInvitationByID(ctx, outbox.ResourceID)
		if err != nil {
			if errors.Is(err, ErrNotFound) {
				return false, nil
			}
		...
		return invitation.Status == model.StatusPending && invitation.ExpiresAt.After(now), nil
	case model.NotificationOutboxResourcePasswordReset:
		reset, err := s.repo.FindPasswordResetByID(ctx, outbox.ResourceID)
		if err != nil {
			if errors.Is(err, ErrNotFound) {
				return false, nil
			}
		...
		return reset.Status == model.StatusPending && reset.ExpiresAt.After(now), nil
	case model.NotificationOutboxResourceEmailVerification:
		verification, err := s.repo.FindEmailVerificationByID(ctx, outbox.ResourceID)
		if err != nil {
			if errors.Is(err, ErrNotFound) {
				return false, nil
			}
		...
		return verification.Status == model.StatusPending && verification.ExpiresAt.After(now), nil
	default:
		return false, ErrInvalidInput
	}
}

func (s *service) sendNotificationOutboxNotice(ctx context.Context, outbox *model.NotificationOutbox) error {
	switch outbox.Kind {
	case model.NotificationOutboxKindInvitation:
		return s.notifier.SendInvitation(ctx, InvitationNotice{Email: outbox.Recipient, Token: outbox.Token, URL: outbox.URL})
	case model.NotificationOutboxKindPasswordReset:
		return s.notifier.SendPasswordReset(ctx, PasswordResetNotice{Email: outbox.Recipient, Token: outbox.Token, URL: outbox.URL})
	case model.NotificationOutboxKindEmailVerification:
		return s.notifier.SendEmailVerification(ctx, EmailVerificationNotice{Email: outbox.Recipient, Token: outbox.Token, URL: outbox.URL})
	default:
		return ErrInvalidInput
	}
}

func (s *service) markNotificationOutboxSent(ctx context.Context, outbox *model.NotificationOutbox) error {
	now := s.now()
	outbox.Status = model.NotificationOutboxStatusSent
	outbox.SentAt = &now
	outbox.LastError = ""
	outbox.UpdatedAt = now
	return s.repo.SaveNotificationOutbox(ctx, outbox)
}

func (s *service) markNotificationOutboxFailure(ctx context.Context, outbox *model.NotificationOutbox, cause error, terminal bool) error {
	retryInterval, maxAttempts := s.currentNotificationRetryPolicy()
	now := s.now()
	if outbox.MaxAttempts <= 0 {
		outbox.MaxAttempts = maxAttempts
	}
	outbox.Attempts++
	if terminal || outbox.Attempts >= outbox.MaxAttempts {
		outbox.Status = model.NotificationOutboxStatusFailed
	} else {
		outbox.Status = model.NotificationOutboxStatusPending
		outbox.NextAttemptAt = now.Add(retryInterval)
	}
	outbox.LastError = truncateNotificationOutboxError(cause)
	outbox.UpdatedAt = now
	return s.repo.SaveNotificationOutbox(ctx, outbox)
}

func (s *service) currentNotificationRetryPolicy() (time.Duration, int) {
	s.notificationMu.RLock()
	interval := s.notificationRetryInterval
	maxAttempts := s.notificationRetryMaxAttempts
...
func truncateNotificationOutboxError(err error) string {
	if err == nil {
		return ""
	}
