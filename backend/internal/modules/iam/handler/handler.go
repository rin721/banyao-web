...
func (h *Handler) ListNotificationOutbox(c ports.HTTPContext) {
	principal, ok := requirePrincipal(c)
	if !ok {
		return
	}
	filter, ok := parseNotificationOutboxFilter(c)
	if !ok {
		return
	}
	page, err := h.service.ListNotificationOutbox(c.RequestContext(), principal, filter)
	h.write(c, page, err)
}

func (h *Handler) RetryNotificationOutbox(c ports.HTTPContext) {
	principal, ok := requirePrincipal(c)
	if !ok {
		return
	}
	id, ok := parseInt64Param(c, "outboxId")
	if !ok {
		return
	}
	item, err := h.service.RetryNotificationOutbox(c.RequestContext(), service.RetryNotificationOutboxInput{
		Principal: principal,
		OutboxID:  id,
		UserAgent: c.GetHeader("User-Agent"),
		IPAddress: c.ClientIP(),
...
func (h *Handler) RevokeSession(c ports.HTTPContext) {
	...
	h.write(c, map[string]bool{"revoked": true}, h.service.RevokeSession(c.RequestContext(), service.RevokeSessionInput{
		Principal: principal,
		SessionID: id,
		UserAgent: c.GetHeader("User-Agent"),
		IPAddress: c.ClientIP(),
...
func parseAuditLogFilter(c ports.HTTPContext) (service.AuditLogFilter, bool) {
	...
	if raw := query.Get("limit"); raw != "" {
		...
		if err != nil {
			badRequestInvalidNumber(c, "limit")
	...
	if raw := query.Get("userId"); raw != "" {
		...
		if err != nil {
			badRequestInvalidNumber(c, "userId")
			return service.AuditLogFilter{}, false
		}
	...
	if raw := query.Get("cursor"); raw != "" {
		parsed, err := strconv.ParseInt(raw, 10, 64)
		if err != nil {
			badRequestInvalidNumber(c, "cursor")
			return service.AuditLogFilter{}, false
		}
...
func parseAPITokenFilter(c ports.HTTPContext) (service.APITokenFilter, bool) {
	...
	if raw := query.Get("page"); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil {
			badRequestInvalidNumber(c, "page")
	...
	if raw := query.Get("pageSize"); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil {
			badRequestInvalidNumber(c, "pageSize")
			return service.APITokenFilter{}, false
		}
...
// parseNotificationOutboxFilter 从查询参数构造 IAM 通知 outbox 过滤条件。
func parseNotificationOutboxFilter(c ports.HTTPContext) (service.NotificationOutboxFilter, bool) {
	query := c.Request().URL.Query()
	filter := service.NotificationOutboxFilter{
	...
	if raw := query.Get("page"); raw != "" {
		...
		if err != nil {
			badRequestInvalidNumber(c, "page")
			return service.NotificationOutboxFilter{}, false
		}
		filter.Page = parsed
	}
	if raw := query.Get("pageSize"); raw != "" {
...
func badRequestInvalidField(c ports.HTTPContext, name string) {
	result.BadRequest(c, "validation.common.invalid", map[string]any{"field": name})
}

func badRequestInvalidNumber(c ports.HTTPContext, name string) {
